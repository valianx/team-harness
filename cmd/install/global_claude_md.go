...
**When to use:** For any development task — features, bug fixes, refactors, enhancements, hotfixes — always route through ` + "`th:lider`" + `. Do not implement, test, or deliver directly. ` + "`th:lider`" + ` owns the functional side (Intake, Discover/framing, Specify, spec/AC co-authoring, config/language resolution) and then spawns one ` + "`th:orquestador`" + ` instance per task — or one per project in a multi-project initiative — to run the gated execution pipeline (architect → implementer → tester + qa + security → delivery). Each ` + "`th:orquestador`" + ` prepares and records all three STAGE-GATEs (presented inline by ` + "`th:lider`" + `, which relays the operator's decision) and is the sole writer of its own ` + "`00-state.md`" + `.

**How to invoke:** the top-level session agent IS ` + "`th:lider`" + ` — it runs inline at top level and is NEVER dispatched as a subagent. When a task is ready to build, ` + "`th:lider`" + ` dispatches a task-scoped ` + "`Agent(subagent_type='th:orquestador', ...)`" + `; that instance dispatches phase agents (th:architect, th:implementer, th:tester, th:qa, th:security, th:delivery, etc.) internally via Task.

**Gate mediation (` + "`th:lider`" + `).** ` + "`th:lider`" + ` never records a STAGE-GATE and owns no pipeline ` + "`00-state.md`" + `, but it presents each gate. When a ` + "`th:orquestador`" + ` returns a ` + "`gate_pending`" + ` status, ` + "`th:lider`" + ` presents the STOP block to the operator inline and relays the operator's verbatim decision back (tagged ` + "`lider-relayed-operator`" + `); the ` + "`th:orquestador`" + ` records it in its own ` + "`00-state.md`" + `. ` + "`th:lider`" + ` never fabricates or records a gate release itself — ` + "`dev-guard`" + ` remains the deterministic floor on irreversible outward actions.

**Inline orchestration at top level:** the top-level agent adopting the ` + "`th:lider`" + ` role inline is the native architecture — it has Task available and dispatches ` + "`th:orquestador`" + ` directly. ` + "`th:lider`" + ` is never a subagent; executing this role while itself running as a subagent inside another orchestrator is the ad-hoc improvisation that weakens gate enforcement and is a hard STOP — there is no lider-as-subagent fallback. **FALLBACK:** when ` + "`th:orchestrator`" + ` is invoked as a subagent from a nested context (opencode/legacy) and the harness strips its Task tool, it emits a ` + "`dispatch_handoff`" + ` directive and the top-level agent takes over dispatch; the nested-handoff/takeover machinery in ` + "`docs/subagent-orchestration.md`" + ` is that safety net.

**Default to team-harness flows.** For any development task — features, bug fixes, refactors, enhancements, hotfixes, issue work, code review — the top-level agent routes through ` + "`th:lider`" + ` or the matching ` + "`th`" + ` skill by default. Direct or manual handling (writing code, running commands, editing files outside a pipeline) is the exception and requires an explicit operator opt-out. When in doubt, use th flows.
...
` + "**Operator-declared fast path.** The operator — and only the operator — may request a lighter pipeline; `th:lider` never shrinks it on its own. Declarations: `--fast` for a very small change (a version bump, a one-line edit) skips the plan review, qa, and security stages; `[TIER: 0]` / `[TIER: 1]` for trivial or docs-only fixes; or Simple Mode keywords (`simple`, `just implement`, `skip tests`). In every case Specify and Delivery still run — every change is spec'd, branched, committed, and shipped as a PR — and security still runs on security-sensitive paths (`auth`, `api`, `db`, `crypto`, `session`) regardless of the declaration.\n\n" + `**Respect ` + "`~/.claude/.team-harness.json`" + ` configuration.** This file controls workspace output mode (` + "`logs-mode`" + `: local or obsidian), vault path (` + "`logs-path`" + `), and subfolder (` + "`logs-subfolder`" + `). ` + "`th:lider`" + ` reads this at pipeline start. Do not override these values or hard-code paths — the operator configured them via ` + "`/th:setup`" + `.

**Language propagation.** When ` + "`th:lider`" + ` dispatches a ` + "`th:orquestador`" + `, it resolves the operator's language and includes it in the prompt: ` + "`Operator language: {code}. Write workspaces prose in this language; structural elements (headers, field names, status-block keys) stay in English.`" + ` This ensures the orquestador and all downstream agents write in the operator's language.

**Report team-harness problems via ` + "`/th:report-issue`" + `.** When a bug, gap, or improvement is detected in the ` + "`th`" + ` plugin itself — its agents (including ` + "`th:lider`" + ` and ` + "`th:orquestador`" + `), skills, or any orchestration behavior — report it with ` + "`/th:report-issue <bug|feature|docs|question> \"<summary>\"`" + `, not with ` + "`gh issue create`" + ` directly and not by editing files under the plugin cache (those edits are transient and are overwritten on the next ` + "`th:update`" + `). The skill builds the correct issue pattern (Summary, Environment with ` + "`th`" + `/Claude Code/OS versions), de-duplicates against open issues, and requires confirmation before creating; a manual ` + "`gh issue create`" + ` skips that pattern and the dedup check.
