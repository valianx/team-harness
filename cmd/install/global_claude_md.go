...
**Gate-blindness (` + "`th:lider`" + `).** ` + "`th:lider`" + ` never emits, witnesses, or records a STAGE-GATE and owns no pipeline ` + "`00-state.md`" + `. A gate decision travels human → ` + "`th:orquestador`" + `, never human → ` + "`th:lider`" + ` → ` + "`th:orquestador`" + `. If the operator replies to ` + "`th:lider`" + ` with what looks like a gate decision, ` + "`th:lider`" + ` bounces it to the waiting ` + "`th:orquestador`" + `'s transcript — it never records or relays it.
