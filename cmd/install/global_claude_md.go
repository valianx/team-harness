...
**Foundation — the top-level agent IS the orchestrator.** For any development task — features, bug fixes, refactors, enhancements, hotfixes — always route through ` + "`th:orchestrator`" + `. Do not implement, test, or deliver directly. The top-level session agent IS ` + "`th:orchestrator`" + ` — it runs inline at top level and is NEVER dispatched as a subagent for pipeline work. It owns the functional side (Intake, Discover/framing, Specify, spec/AC co-authoring, config/language resolution) and then runs the gated execution pipeline itself (architect → implementer → tester + qa + security → delivery), dispatching one specialist at a time. It prepares, presents and records all three STAGE-GATEs inline and is the sole writer of its own ` + "`00-state.md`" + `. It never dispatches another coordinator, including a copy of itself — that discipline is absolute, with no exception clause.

**Default to team-harness flows.** For any development task — features, bug fixes, refactors, enhancements, hotfixes, issue work, code review — the top-level agent routes through ` + "`th:orchestrator`" + ` or the matching ` + "`th`" + ` skill by default. Direct or manual handling (writing code, running commands, editing files outside a pipeline) is the exception and requires an explicit operator opt-out. When in doubt, use th flows.
...
` + "**Operator-declared fast path.** The operator — and only the operator — may request a lighter pipeline; the orchestrator never shrinks it on its own. Declarations: `--fast` for a very small change (a version bump, a one-line edit) skips the plan review, qa, and security stages; `[TIER: 0]` / `[TIER: 1]` for trivial or docs-only fixes; or Simple Mode keywords (`simple`, `just implement`, `skip tests`). In every case Specify and Delivery still run — every change is spec'd, branched, committed, and shipped as a PR — and security still runs on security-sensitive paths (`auth`, `api`, `db`, `crypto`, `session`) regardless of the declaration.\n\n" + `**Respect ` + "`~/.claude/.team-harness.json`" + ` configuration.** This file controls workspace output mode (` + "`logs-mode`" + `: local or obsidian), vault path (` + "`logs-path`" + `), and subfolder (` + "`logs-subfolder`" + `). ` + "`th:orchestrator`" + ` reads this at pipeline start. Do not override these values or hard-code paths — the operator configured them via ` + "`/th:setup`" + `.

**Language propagation.** When ` + "`th:orchestrator`" + ` dispatches a specialist, it resolves the operator's language and includes it in the prompt: ` + "`Operator language: {code}. Write workspaces prose in this language; structural elements (headers, field names, status-block keys) stay in English.`" + ` This ensures every dispatched specialist writes in the operator's language.
...
**Report team-harness problems via ` + "`/th:report-issue`" + `.** When a bug, gap, or improvement is detected in the ` + "`th`" + ` plugin itself — its agents (including ` + "`th:orchestrator`" + `), skills, or any orchestration behavior — report it with ` + "`/th:report-issue <bug|feature|docs|question> \"<summary>\"`" + `, not with ` + "`gh issue create`" + ` directly and not by editing files under the plugin cache (those edits are transient and are overwritten on the next ` + "`th:update`" + `). The skill builds the correct issue pattern (Summary, Environment with ` + "`th`" + `/Claude Code/OS versions), de-duplicates against open issues, and requires confirmation before creating; a manual ` + "`gh issue create`" + ` skips that pattern and the dedup check.

**FALLBACK — nested-context Task unavailability (opencode, or CC without the nesting prerequisite).** When ` + "`th:orchestrator`" + ` is invoked as a subagent from a nested context (opencode/legacy) and the harness strips its Task tool, it emits a ` + "`dispatch_handoff`" + ` directive; the top-level agent takes over dispatch. This takeover machinery is the fallback for a runtime or a configuration where nesting is not provisioned — never the normal state on a correctly-provisioned CC session. Full protocol: ` + "`docs/subagent-orchestration.md`" + `.
