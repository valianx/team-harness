...
This is the opencode projection of the canonical Team Harness `todo`
...
1. Treat the live operator request for `todo` as the canonical
   `$ARGUMENTS` value. Examples written as `/th:todo` name the same capability; interpret them as examples rather than requiring Claude Code.
...
6. Store persistent values in opencode-native Team Harness configuration and preserve unrelated keys.
