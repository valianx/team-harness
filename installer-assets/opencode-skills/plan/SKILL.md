...
This is the opencode projection of the canonical Team Harness `plan`
...
1. Treat the live operator request for `plan` as the canonical
   `$ARGUMENTS` value. Examples written as `/th:plan` name the same capability; interpret them as examples rather than requiring Claude Code.
