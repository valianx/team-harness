...
Recover the evidence, workspace, review decision and any explicit operator stop
or decline. Every completed repository-file change uses create-pr preparation,
even when no PR was initially requested; read-only work and outputs kept outside
the repository have no candidate to prepare. Publication follows native
permissions and honors an explicit stop or decline. Preparation does not imply
publication.
...
Use [create-pr](../create-pr/SKILL.md) for PR preparation and publication for
every repository candidate, including OpenSpec archive and transient-file
hygiene. A delivery specialist may draft
