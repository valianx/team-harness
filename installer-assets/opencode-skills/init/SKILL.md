...
This is the opencode projection of the canonical Team Harness `init`
...
1. Treat the live operator request for `init` as the canonical
   `$ARGUMENTS` value. Examples written as `/th:init` name the same capability; interpret them as examples rather than requiring Claude Code.
