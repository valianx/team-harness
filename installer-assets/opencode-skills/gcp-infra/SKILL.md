...
This is the opencode projection of the canonical Team Harness `gcp-infra`
...
1. Treat the live operator request for `gcp-infra` as the canonical
   `$ARGUMENTS` value. Examples written as `/th:gcp-infra` name the same capability; interpret them as examples rather than requiring Claude Code.
...
6. Keep read-and-plan as the default. Treat apply intent as gated and require a fresh explicit operator approval before mutation.
