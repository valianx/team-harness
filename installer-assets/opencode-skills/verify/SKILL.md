...
This is the opencode projection of the canonical Team Harness `verify`
...
1. Treat the live operator request for `verify` as the canonical
   `$ARGUMENTS` value. Examples written as `/th:verify` name the same capability; interpret them as examples rather than requiring Claude Code.
