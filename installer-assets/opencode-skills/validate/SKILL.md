...
This is the opencode projection of the canonical Team Harness `validate`
...
1. Treat the live operator request for `validate` as the canonical
   `$ARGUMENTS` value. Examples written as `/th:validate` name the same capability; interpret them as examples rather than requiring Claude Code.
