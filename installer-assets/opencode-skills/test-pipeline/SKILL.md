...
This is the opencode projection of the canonical Team Harness `test-pipeline`
...
1. Treat the live operator request for `test-pipeline` as the canonical
   `$ARGUMENTS` value. Examples written as `/th:test-pipeline` name the same capability; interpret them as examples rather than requiring Claude Code.
