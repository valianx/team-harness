...
This is the opencode projection of the canonical Team Harness `pipelines`
...
1. Treat the live operator request for `pipelines` as the canonical
   `$ARGUMENTS` value. Examples written as `/th:pipelines` name the same capability; interpret them as examples rather than requiring Claude Code.
...
6. Resolve workspace settings from opencode-native Team Harness configuration and render state read-only.
