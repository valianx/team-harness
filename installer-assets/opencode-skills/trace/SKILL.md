...
This is the opencode projection of the canonical Team Harness `trace`
...
1. Treat the live operator request for `trace` as the canonical
   `$ARGUMENTS` value. Examples written as `/th:trace` name the same capability; interpret them as examples rather than requiring Claude Code.
...
6. Resolve workspace and pricing settings from opencode-native Team Harness configuration and remain read-only.
