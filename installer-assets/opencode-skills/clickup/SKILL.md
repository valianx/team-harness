...
This is the opencode projection of the canonical Team Harness `clickup`
...
1. Treat the live operator request for `clickup` as the canonical
   `$ARGUMENTS` value. Examples written as `/th:clickup` name the same capability; interpret them as examples rather than requiring Claude Code.
...
6. Store persistent values in the opencode-native Team Harness configuration. Preserve all opaque keys and never print credentials.
