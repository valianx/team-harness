...
This is the opencode projection of the canonical Team Harness `report-issue`
...
1. Treat the live operator request for `report-issue` as the canonical
   `$ARGUMENTS` value. Examples written as `/th:report-issue` name the same capability; interpret them as examples rather than requiring Claude Code.
