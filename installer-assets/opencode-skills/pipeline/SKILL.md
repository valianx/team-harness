...
This is the opencode projection of the canonical Team Harness `pipeline`
...
1. Treat the live operator request for `pipeline` as the canonical
   `$ARGUMENTS` value. Examples written as `/th:pipeline` name the same capability; interpret them as examples rather than requiring Claude Code.
