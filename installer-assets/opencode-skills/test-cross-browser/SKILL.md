...
This is the opencode projection of the canonical Team Harness `test-cross-browser`
...
1. Treat the live operator request for `test-cross-browser` as the canonical
   `$ARGUMENTS` value. Examples written as `/th:test-cross-browser` name the same capability; interpret them as examples rather than requiring Claude Code.
