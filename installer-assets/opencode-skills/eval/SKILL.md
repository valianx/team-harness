...
This is the opencode projection of the canonical Team Harness `eval`
...
1. Treat the live operator request for `eval` as the canonical
   `$ARGUMENTS` value. Examples written as `/th:eval` name the same capability; interpret them as examples rather than requiring Claude Code.
