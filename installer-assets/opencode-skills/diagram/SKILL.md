...
This is the opencode projection of the canonical Team Harness `diagram`
...
1. Treat the live operator request for `diagram` as the canonical
   `$ARGUMENTS` value. Examples written as `/th:diagram` name the same capability; interpret them as examples rather than requiring Claude Code.
...
6. Create the diagram through the packaged Excalidraw workflow; do not dispatch a Claude agent.
