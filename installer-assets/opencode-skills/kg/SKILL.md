...
This is the opencode projection of the canonical Team Harness `kg`
...
1. Treat the live operator request for `kg` as the canonical
   `$ARGUMENTS` value. Examples written as `/th:kg` name the same capability; interpret them as examples rather than requiring Claude Code.
...
6. Use the configured Memory MCP when available; otherwise report the missing native dependency.
