...
- **If the change touches distributed plugin assets — `agents/`, `skills/`, or `hooks/` — write a `changelog.d/{pr-slug}.md` fragment** (same convention as the CHANGELOG above; slug `[a-z0-9-]+`). The plugin version bump is **deferred to release-time**: run `/th:release` when you are ready to cut a release, which aggregates all pending `changelog.d/` fragments (and any `version.d/` markers for fragment-less internal bumps), bumps all three version sites once (`.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json plugins[0].version`, `CLAUDE.md §3`), and assembles the CHANGELOG. Until the release is cut, `claude plugin update` sees no version change — operators receive updates at release cadence, not per-PR. Editing an `agents/*.md` cold-invalidates that agent's Claude Code cached prefix for every operator until the session re-warms it, so batch agent edits into one release rather than dribbling them out (see `docs/cost-and-caching.md`). For urgent/security changes, run `/th:release` immediately after the merge to ship without waiting for other pending work. For a fragment-less internal change that the consumer receives (a refactor the consumer observes), write a `version.d/{pr-slug}.bump` marker (one line: `patch`, `minor`, or `major`) instead of a `changelog.d/` fragment; the release step folds it into the bump-level max.
...
- **2026-06-29** — `refreshManagedConfigKeys` vs `writeOpencodeTeamHarnessConfig`: update overwrites only `format_version`/`installed_version`/`updated_at`, never forces `logs-mode`/`logs-path`. → `cmd/install/opencode_config.go`
- **2026-06-29** — `VERSION` asset for version pre-check: bare semver at `releases/latest/download/VERSION` avoids GitHub API. Best-effort; bootstrap falls through to binary download if absent. → `release.yml`
...
<!-- Populated by the delivery agent after each feature. Empty at init. -->
- **VERSION pre-check is best-effort**: not SHA-signed; MITM can suppress an update. Binary SHA256 verify is the floor. (SEC-OC-U-01, Low)
- **opencode requires restart for asset changes**: hot-reload is experimental-flag-only. Confirmed via issues #10899/#8751.
