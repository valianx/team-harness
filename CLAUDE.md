...
**Current version:** `2.45.0` (see `.claude-plugin/plugin.json` `version` field — canonical source of truth for the plugin marketplace. `CHANGELOG.md` tracks the release history).
...
- **Discover phase + intake survey.** Default intake is conversational — the architect fires only on an explicit advance signal, never on message arrival; a fast-path offers one confirmation for a clear task. The intake survey captures meta-decisions (shape, effort, autonomy, scope-hint) as attributable `00-state.md` answers. Depth DIAL, not a stage switch; security floors non-surveyable. Full contract: `docs/discover-phase.md`.
