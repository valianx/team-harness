...
**Current version:** `2.143.1` (see `.claude-plugin/plugin.json` `version` field — canonical source of truth for the plugin marketplace. `CHANGELOG.md` tracks the release history).
...
- **2026-07-28** — Host-runtime write refusal is distinct from operator decline (#544): when an agent's write to `~/.claude/settings.json` is refused by the host runtime, the step never records a durable decline (`nested_spawn_depth.declined`); only an explicit `n` produces that durable state, so a transient environment failure re-offers the gate next run instead of permanently suppressing it. → `docs/setup-update-model.md § "Architecture prerequisite: subagent nesting depth"`
...
- **Guard-on-the-guard needs liveness, not presence** (#544): a content-anchor test verifying only that a literal/regex is present in security-critical script text does not detect an additive edit that neutralizes the guarded logic while every anchor still matches; prefer hash-pinning (Suite 174's sha256 idiom) or behavioral execution when the guard must prove exclusivity/liveness. → `tests/test_agent_structure.py` Suite 173 vs Suite 174
...
**FALLBACK — nested-handoff/takeover (opencode, or CC without the nesting prerequisite).** Claude Code's subagent-nesting depth is configurable, not a permanent cap, via `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` in `~/.claude/settings.json` (defaults to unset; `/th:setup`/`/th:update` provision `"2"` — `docs/setup-update-model.md § Architecture prerequisite: subagent nesting depth`). `dispatch_handoff` takeover is the fallback for a runtime or a configuration where nesting is not provisioned — opencode always, or CC before that prerequisite is set — never the normal state on a correctly-provisioned CC session. Full protocol: `docs/subagent-orchestration.md`.
