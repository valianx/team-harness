...
**The top-level agent IS the orchestrator.** No filesystem marker is required. Its 2,528-word kernel serves direct work without reading the pipeline reference, whose ceiling is 1,647 words (both ceilings recorded in `tests/fixtures/authoring-baseline.json`); after explicit activation, the same coordinator loads phase sections and dispatches specialists. It never dispatches another coordinator. `dev-guard` gates the minimal outward floor regardless of posture.
