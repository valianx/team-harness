...
**Current version:** `2.44.0` (see `.claude-plugin/plugin.json` `version` field — canonical source of truth for the plugin marketplace. `CHANGELOG.md` tracks the release history).
...
- **Patch mode + selective verifier re-run.** When a verifier fails and blast radius is `localized`, the producer edits only named elements and the orchestrator re-runs only the affected verifier domain; a mandatory coherence gate runs post-patch. Default is `structural` (full re-dispatch). Full contract: `docs/patch-mode.md`.
