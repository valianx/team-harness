...
- **Internal distribution rule of the team-harness repository (this bullet does NOT apply to consumers of the shipped pipeline — the shipped `delivery`/`orchestrator` default bumps the project version once per PR; see `agents/delivery.md § Step 9`).** If the change touches distributed plugin assets — `agents/`, `skills/`, or `hooks/` — write a `changelog.d/{pr-slug}.md` fragment (same convention as the CHANGELOG above; slug `[a-z0-9-]+`). team-harness's own plugin version bump is **deferred to release-time**: the orchestrator passes `skip-version: true` for team-harness deliveries (the repo-local deferral convention the shipped `delivery.md` escape-hatch honors), and `/th:release` is run when ready to cut a release, aggregating all pending `changelog.d/` fragments (and any `version.d/` markers for fragment-less internal bumps), bumping all three version sites once (`.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json plugins[0].version`, `CLAUDE.md §3`), and assembling the CHANGELOG. Until the release is cut, `claude plugin update` sees no version change — operators receive updates at release cadence, not per-PR. Editing an `agents/*.md` cold-invalidates that agent's Claude Code cached prefix for every operator until the session re-warms it, so batch agent edits into one release rather than dribbling them out (see `docs/cost-and-caching.md`). For urgent/security changes, run `/th:release` immediately after the merge to ship without waiting for other pending work. For a fragment-less internal change that the consumer receives (a refactor the consumer observes), write a `version.d/{pr-slug}.bump` marker (one line: `patch`, `minor`, or `major`) instead of a `changelog.d/` fragment; the release step folds it into the bump-level max. `/th:release` (`skills/release/SKILL.md`) and the three-site bump enumeration are team-harness-internal release tooling — not part of the shipped default a consumer's per-PR delivery invokes.
