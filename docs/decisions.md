...
- **2026-07-21** — Pre-Delivery Security Audit narrowed to `adversary` alone, conditional on `security_floor_applies`: the unconditional `security` code-audit dispatch is removed from Phase 3.8, and code-level review for a non-sensitive task is delegated to PR review (named generically, not tied to any specific configured tool). SEC-002 design-review (Phase 1.6), the `security_floor_applies` predicate, the Phase-2-close backstops, and Phase 4.5 internal review are unchanged. Also retires the Phase 3.6 `acceptance-checker` drift audit entirely (no replacement dispatch). This historical dispatch policy is retired; current workflows select independent review from the task and native permissions govern execution. See `docs/how-it-works.md`.
- **2026-07-27** — Canonical dispatch contract (`agents/_shared/dispatch-contract.md`, issue #524): a single home for what a dispatch prompt may and must not carry — coordinates, task-scope decisions absent from the board, and the return form are permitted; a restatement of the destination agent's own contract, a summary of another agent's work, a verdict on the property under test, and an operator-facing options menu are not. States one two-halves rule: review scope is never bounded by the dispatcher — the recipient computes its own review objective from the coordinates it receives, never from a delegated verdict, with the named exception of being handed an affirmation to invert (e.g. `adversary` receiving the Stage-1 SEC-002 design-review verdict as its own target) — and write scope is always bounded by the recipient's own contract, by pointer to `agents/_shared/plan-consolidation.md § "Write-scope on the plan set (closed list)"` and § "Write-tool discipline (shared review files)", without restating either. Every control the file asserts is declared in a five-column rubric (`Control` | `Enforcer` | `Failure direction` | `Invoker` | `Read at`) rather than left in prose, so an omission is a visible empty cell. → `agents/_shared/dispatch-contract.md`
- **2026-07-27 (superseded 2026-09-22)** — Historical per-specialist commit contract (verifiable-contracts, #528). Main now coordinates Git by default; a specialist commits only with assigned nonoverlapping ownership. → `agents/implementer.md § "Git and workspace"`, `agents/ref-pipeline.md`
