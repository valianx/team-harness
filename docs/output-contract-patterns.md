...
| `01-plan.md` | Compact projection of the bound OpenSpec change (`agents/ref-pipeline.md § "Coordinated development workflow"`): outcome, scope, batches, risks, decisions, canonical links, pinned identity; no copied AC/TC prose, task graph, or commands. Above target emit `size_reason: required-items`. |
...
(`docs/observability.md § "Workflow visibility"`) as the baseline — no new instrumentation is introduced.
