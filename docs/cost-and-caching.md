...
**team-harness's own version sites (what `/th:release` bumps).** Three sites are mandatory and bumped together: `.claude-plugin/plugin.json` `"version"` field, `.claude-plugin/marketplace.json` `plugins[0].version` (NOT the schema/top-level `"version"` field, a different field that must never be touched), and `CLAUDE.md §3` `**Current version:**` line. `cmd/install/main.go`'s `var version` literal is a separate `legacy-installer` anchor — updated only on Go-installer releases, not on plugin-asset-only changes. `CHANGELOG.md` receives the release heading (`## [X.Y.Z] - YYYY-MM-DD`) at release cut, assembled from pending `changelog.d/` fragments.

**Single-PR release path opt-in (`--with <feature-branch>`).** `/th:release --with <feature-branch>` folds the same three-site bump + `changelog.d/` assembly into a feature branch's own PR instead of opening a second `release/vX.Y.Z` PR — one PR, one CI run, one cache cold-start instead of two. `hooks/ts/bodies/prepublish-guard.ts` recognizes an in-tree `version.d/.release-cut` marker (or a `release-cut: vX.Y.Z` commit trailer as a secondary signal) to decouple the release-path check from the `release/vX.Y.Z` branch-name convention; the three-site invariant itself is unchanged — the marker authorizes running the check, never bypassing it, and a marker whose content is not strict SemVer denies the push. The batch model above (no flag) is retained unchanged and remains the better choice when several PRs have accumulated pending work between releases; the single-PR path is an additional option for a single feature that should ship as its own release. See `skills/release/SKILL.md § Mode — Inline release` and `agents/delivery.md § Step 9` (inline-release row).

**Bootstrap note.** This cache-invalidation-hygiene build ships via the existing deferred cut (`skip-version: true` + a `changelog.d/` fragment) rather than the single-PR path it introduces — the marker-recognition hook is not active until it is installed on the operator's machine. The single-PR path applies starting with the first release cut after this build is installed.
...
- **The operator's top-level session.** The leader runs at the top level. On a subscription its session gets the 1-hour TTL, so the system prompt and CLAUDE.md stay cached across the full pipeline run.

## Lane cost model — th:leader's orchestrator fan-out

th:leader multiplies th:orchestrator — one lane per task in a same-repo batch, one per project in a multi-project initiative (`agents/leader.md § Multi-Task fan-out`, `§ Repo-identity verification and orchestrator multiplication`). Each lane is a fully isolated subagent conversation with its own system prompt, tool set, and cache; nothing in one lane's cache is visible to another.

**5-minute TTL, and re-cost after a gate pause.** Every orchestrator runs as a subagent, so its cache uses the 5-minute write TTL — not the 1-hour TTL the top-level session may hold (`§ Subagents and cost`). A STAGE-GATE pause is a wall-clock stop of unbounded length: when a lane's orchestrator is parked at STAGE-GATE-1/2/3 waiting for the operator's panel reply, and that wait exceeds 5 minutes, the lane's 5-minute cache has already expired by the time the gate is released. The first turn after the release is therefore a cold write again, not a warm read — re-cost every gate resumption that idled longer than the 5-minute window as a fresh cache creation, not as a hit. An operator who expects long deliberation at a gate can opt the whole session into the 1-hour TTL (`ENABLE_PROMPT_CACHING_1H`, or the subscription-automatic 1-hour TTL on the top-level session), but the subagent lanes themselves stay on the 5-minute floor regardless — the 1-hour TTL is a main-conversation property.

**N concurrent lanes → N cold starts, and the opus tier does not amortize.** The orchestrator itself is `model: sonnet`, so N lanes are N sonnet cold starts. The expensive cold start is one tier down: every lane's orchestrator dispatches its own `architect` (`model: opus`) at Phase 1 Design, plus opus-tier `security` / `plan-reviewer` work at Phase 1.6. The re-dispatch warm-reuse window (`§ Subagents and cost` — same agent type within 5 minutes reuses the warm prefix) does NOT rescue concurrent lanes: lanes launched in the same round start their architect dispatches simultaneously, so none has written a cache the others can read yet. N lanes launched together therefore pay N independent opus cold-start cache-creations, not one cold + (N−1) warm — cost scales with lane count at the opus tier, not sub-linearly. Serial dispatch (`--serial` / "one at a time") is the case where the 5-minute reuse window CAN help: a second lane's architect dispatched within 5 minutes of the first reuses the warm opus prefix.

**The fan-out confirmation surfaces lane count + cost before approval.** th:leader never spawns N ≥ 2 lanes silently. Before dispatching a multi-lane fan-out (multi-task or multi-project), it shows the operator the lane count and an approximate cost estimate and waits for explicit confirmation (`agents/leader.md § Repo-identity verification` — "Fan-out confirm surfaces lane count + cost"). This is a dispatch-count decision the leader holds itself; it is NOT a STAGE-GATE (the leader never records a gate). `--serial` bypasses the confirm and runs lanes sequentially.
