...
**Batching agent edits per release (superseded — history kept for context, not the current model).** each distributed agent file (`agents/*.md`) that changes cold-invalidates that agent's cached prefix for every operator whose Claude Code session loaded it. team-harness previously deferred its own plugin-version bump to a separate release cut (`/th:release`), so feature PRs accumulated changes in `changelog.d/` without bumping the version and operators only paid one batch cache-cold-start per release instead of one per PR. That deferred model is **retired**: team-harness now bumps its own project version once per PR during implementation assembly, before Freeze (see `CLAUDE.md §6.3` and `agents/_shared/implementation-assembly.md`). The batching saving was real but modest, and the operator's workflow-cost judgment — the release-cut ceremony (a dedicated tool, a marker/trailer discriminator, a second review pass) cost more in process overhead than the batched cache-cold-start saved in per-session warm-up — is why it was superseded rather than kept as an opt-in.

**team-harness's own version sites (bumped per PR during implementation).** The current tree has five synchronized sites: `.claude-plugin/plugin.json` `"version"` field (canonical), `.claude-plugin/marketplace.json` `plugins[0].version` (NOT the schema/top-level `"version"` field, a different field that must never be touched), `plugins/team-harness/.codex-plugin/plugin.json`, `CLAUDE.md §3` `**Current version:**` line, and `cmd/install/main.go`'s fallback `var version` literal. The fallback is checked in CI and release validation so a binary built without `-ldflags` cannot advertise a stale release. Repositories predating the Codex plugin or installer site retain the historical optional-site compatibility until the path exists. `CHANGELOG.md` receives the `## [X.Y.Z] - YYYY-MM-DD` heading directly in that same PR before Freeze (`agents/_shared/implementation-assembly.md`). `changelog.d/` is batch/fallback-only here: use it only when a session intentionally groups changes before one cut, never as team-harness's normal per-PR publication path.
...
- **Cold start per agent.** On the first turn of each dispatched agent (architect, implementer, tester, cleaner, qa, security, delivery), there are no cache hits. The agent warms its own cache across its own subsequent turns.
