...
- **Approach checkpoint (1.4, Variant B):** the architect emits `### Proposed Approach` first and declares `approach_freedom: high|low` in the status block. The orchestrator gates on that declaration: `low` → auto-confirm; `high` → one lightweight STOP.
...
### 2.1 When the orchestrator offers seeding

During Discover open (Step 6d, `docs/discover-phase.md §4`), after the advance signal and intake survey but before dispatching the architect, the orchestrator offers the operator a chance to seed the spec:
...
The scope hint captured in the intake survey (`survey_scope_hint`) serves as a fifth, lighter seed — it seeds file scope and is already written to `00-state.md`. The orchestrator passes it to the architect without re-asking.

### 2.2 The `00-spec-seed.md` artefact

When the operator provides any seeding response, the orchestrator writes `{docs_root}/00-spec-seed.md`. Format:
...
When `00-spec-seed.md` is written, the orchestrator sets in `00-state.md § Current State`:
...
`spec_seed_dissents` is written by the orchestrator AFTER reading the architect's status block: `true` when the status block declares `spec_seed_dissent: true`; `false` otherwise.
...
The orchestrator adds to the in-memory dispatch payload (alongside the existing 9 fields):
...
The orchestrator reads the architect's status block after Phase 1 and sets `spec_seed_dissents` in `00-state.md` accordingly.
...
STAGE-GATE-1 copies `## Review Summary` verbatim into the STOP block (`orchestrator.md:1411`). The dissent must be where the operator already looks — inline markers in the plan body would be missed at the gate.
...
| `approach_freedom` | Orchestrator action |
|--------------------|---------------------|
...
This line is non-blocking (same pattern as `1.7-ux-enrich` at `orchestrator.md:535`): it is marked `[~auto-confirmed: ...]` when the architect declared `low`, never blocks the Phase Checklist gate.
...
```jsonl
{"ts":"…","event":"phase.start","feature":"…","phase":"1.0-approach-check","agent":"orchestrator"}
{"ts":"…","event":"phase.end","feature":"…","phase":"1.0-approach-check","agent":"orchestrator","status":"auto-confirmed|confirmed|adjusted","extra":{"approach_freedom":"low|high"}}
```
...
- The orchestrator dispatches the architect with `mode: approach-sketch` (reduced effort), producing ONLY `### Proposed Approach` and declaring grados-de-libertad.
