...
|---|---|---|---|
| **author** | Single source: `agents/*.md`, `skills/*.md`, `hooks/ts/bodies/*.ts` + `hooks/ts/entry/*.cc.ts`. | Agents and skills use the canonical bodies. Agents/commands receive an emit-time name/model/mode delta; the four PR review agents additionally receive a deny-by-default read/glob/grep permission map. Other per-agent permissions remain omitted and no Team Harness hook plugin is installed. | asymmetric |
