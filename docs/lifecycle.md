...
|---|---|---|---|
| **author** | Single source: `agents/*.md`, `skills/*.md`, `hooks/ts/bodies/*.ts` + `hooks/ts/entry/*.cc.ts`. | Agents and skills use the canonical bodies. Agents/commands receive an emit-time name/model/mode delta applied by `opencodeRuntimeTransform`; per-agent permissions are omitted and no Team Harness hook plugin is installed. | asymmetric |
...
| **test** | `tests/run-all.sh` exercises the wired Claude Code hooks and retained body-level regression suites. | Go tests cover installation, preservation, data-home, ledger, agent projection, and default-agent configuration. | asymmetric |
