...
|---|---|---|---|
| **author** | Single source: `agents/*.md`, `skills/*/SKILL.md`, and retained Claude session-context/observation assets. | The canonical skill catalog is projected through opencode-native adapters; agents/commands receive their existing emit-time deltas. The four PR review agents additionally receive a deny-by-default read/glob/grep permission map. No Team Harness policy-hook layer is installed. | asymmetric |
| **build** | The plugin distributes the git tree as-is, including retained Claude runtime assets. | The Go installer binary is cross-compiled per release (`release.yml`, 5 platform targets, `CGO_ENABLED=0`). | asymmetric |
| **test** | `tests/run-all.sh` exercises retained Claude context/observation behavior and repository validation suites. | Go and Node projection tests cover the four deny-by-default PR agents in addition to installation, preservation, data-home, ledger, and default-agent configuration. | asymmetric |
| **version** | `.claude-plugin/plugin.json` `version` is the canonical site; `.claude-plugin/marketplace.json` `plugins[0].version`, `plugins/team-harness/.codex-plugin/plugin.json`, and `cmd/install/main.go`'s fallback `var version` mirror it (fenced four-site invariant in the current tree). `CLAUDE.md` carries no release version. | The Go binary's `version` var is injected at build time via `-ldflags "-X main.version=..."`, sourced from the same release tag; the checked-in fallback is verified against that tag. | shared (one version namespace, mirrored sites) |
...
| **install — mechanism** | `/plugin marketplace add valianx/team-harness` → `/plugin install th` → `/th:setup` (workspace and native preferences, once). No build, no installer binary — the marketplace plugin is the only CC install channel. | `install-opencode.sh`/`.ps1` downloads the Go binary and forwards `apply --runtime opencode`; `install.sh`/`.ps1`/`.cmd` are compatibility bootstraps that print the native Claude path with no arguments and forward only an explicit subcommand. The plan/apply/uninstall engine keeps its append-only ownership ledger and SEC-01..08 guards. The Go binary does not install Claude Code; a bare invocation prints a marketplace redirect notice. | asymmetric |
...
| **update — managed-context sync** | Every file in the plugin tree is overwritten by the plugin runtime on `/reload-plugins`; fixed-path managed blocks are synced separately by `/th:update`. | `refreshManagedConfigKeys()` updates installer metadata in `.team-harness.json`; `opencode.json` registers the installed TH guide through native `instructions`, preserving agent selection and unrelated entries. Previously owned plugin files are removed through the ledger. | diverged |
| **update — notification** | The update flow reports when a downloaded version needs activation; an already-current run does not request a restart. | The updater reports the changed domains and host activation state. A restart or reconnect is suggested only when opencode reports that reload cannot activate the changed assets; already-current and installed-ahead paths remain no-ops. | asymmetric |
| **activate** | Use `/reload-plugins` after a downloaded version. Reconnect only when the host reports a changed component that cannot activate in place. | Use the native opencode activation path and follow its reported reload or restart requirement for changed assets. | diverged (both require explicit reactivation; the trigger and underlying reason differ) |
| **deprecate** | Old install modes (`standard`/`low-cost`) exist as vestigial plugin-frontmatter concepts only in the historical Go installer code and are not reachable through the marketplace channel; Claude retains only its context/observation integrations. | The Go installer's former CC install path is retired (bare invocation prints a redirect notice); native permissions and approvals remain the action boundary. | asymmetric |
