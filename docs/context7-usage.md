...
If the decision is load-bearing (the API you're using might have changed between training cutoff and now), surface it in your status block via the `context7_consult` counter (see §5) so the orquestador can decide whether to escalate.
