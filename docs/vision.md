...
- **Written intent gives substantial changes a shared reference.** OpenSpec keeps the objective, design, and tasks with the repository. Useful execution notes and recovery state stay in the configured canonical workspace: repository-local for local mode, or directly in the selected vault for Obsidian mode, without a second copy. This lets the developer and collaborators understand decisions and continue the work. The chosen workflow determines how much coordination is useful.
