...
Claude Code's native subagent-nesting depth is configurable, not a permanent cap, via the `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` environment variable in `~/.claude/settings.json`. `th:orchestrator` normally runs AS the top-level session (layer 0) and dispatches its own leaf specialists at layer 1 — no nesting-depth prerequisite is needed for that path. This setting provides headroom for the atypical case where `th:orchestrator` itself is invoked one layer deep (a skill wrapper, an `@`-mention inside an ongoing agent session): without it, that nested instance would not retain the `Task` tool and could not reach its own specialists. Setting the value to `"2"` covers that case (the nested `th:orchestrator` at layer 1, its own specialist dispatches at layer 2) with no further nesting permitted below that. See `docs/subagent-orchestration.md § "Nested-context dispatch — RETIRED protocol, retained provisioning"` for why this is retained provisioning rather than a load-bearing relay.
...
8. **On decline, an aborted corrupted-file write, or a failed write → correctness is not gated on adopting it.** Depth-2 nesting headroom stays useful regardless — a specialist leaf agent invoked one level deep (a skill wrapper, an `@`-mention inside an ongoing session) still benefits from it — but no coordinator-level relay depends on this value being set (`docs/subagent-orchestration.md § "Nested-context dispatch — RETIRED protocol, retained provisioning"`). This mechanism buys back that headroom; it does not gate correctness on adopting it.
...
**Diagnosing a missing prerequisite (not a third provisioning site).** `agents/orchestrator.md § "No capability-check fallback"` names why no boot-time capability check exists any more — the prior two-coordinator handoff check retired with the split, and the coordinator never diagnoses, offers, prompts, or writes this value itself. The two commands above are the only two write sites.
