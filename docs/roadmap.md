...
**What it is.** For every logic-bearing change, translate the approved functional acceptance criteria into a test contract before production code is written. The pre-implementation contract may include reviewed Gherkin, user-visible QA procedures, and an executable failing unit, integration, or acceptance test. Executable runner glue that depends on the implementation may still be built with the implementation, but its scenarios, inputs, outputs, and expected observations must already be fixed. Documentation, asset-only work, and environments where a red test cannot yet execute require an explicit `not-applicable` reason plus a test manifest or inspection procedure; they never silently skip the contract.
