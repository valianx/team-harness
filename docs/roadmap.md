...
**Form (team-harness-native).** Define a small workspace-local quality manifest containing exact command/argument arrays, working directories, supported file scopes, required tool versions or version ranges, coverage and CRAP adapters, and thresholds. The manifest is operational state outside the product diff; when its workspace is nested below a checkout it remains ignored and untracked. The runner emits bounded schema-validated JSON with base and candidate tree identities, command and configuration hashes, tool versions, exit codes, duration, changed paths, selected test identifiers, coverage, complexity, CRAP, and pre/post deltas; missing required tools, malformed output, scope escape, or stale hashes fail closed. Red evidence is valid only when the production surface is unchanged, the approved test identifier fails, and the runner captures the failure; green evidence must reuse identical contract bytes, canonical test binding, effective test runtime, task baseline, and test blobs from the immutable red checkpoint. A non-test manifest change reruns its readiness and Freeze controls without discarding identical RED/GREEN evidence; any test-input replacement starts a new red checkpoint before implementation. The cleaner receives the machine baseline, may edit only its approved surface, and must return a second runner record proving formatter/lint success, focused-test preservation, and the CRAP disposition for every changed/new function. Keep semantic judgments — whether the red failure represents the intended behavior, whether duplication is material, and whether an exception is justified — with fresh bounded agents or the operator, always anchored to the immutable machine record.
