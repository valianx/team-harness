...
> **Superseded in part (rationale only, not the outcome).** This rationale rests on `adversary` having no `Edit` tool. It does now — granted deliberately by the shared-review-file write discipline (#527), which requires every panel writer to amend its own labelled section in place rather than regenerate a file other agents also write. The read-only trust boundary the rationale protects is unchanged, because it was never about `Edit` versus `Write`: `adversary` writes only its own report and its own review section, and touches no source file either way. The per-round-file outcome below still stands on its own merit (a given `N` never accepts a second write), but the "adding `Edit` would be a security regression" argument no longer describes the tree.
