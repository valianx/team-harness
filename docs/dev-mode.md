...
| `gh pr create` | `ask` by default; `allow` under the opt-in config key `autogate.pr_create: true` (`hooks/ts/bodies/dev-guard.ts:743-754`) | Opens a PR — an outward, GitHub-visible action; the opt-in exists for operators who want to remove the double-prompt; the `gate-guard` order floor described below is unwired as of v2.139.0, so this is now the only prompt on that path |
