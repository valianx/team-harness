...
- **th:lider's gate presentation must not oversell this.** When th:lider presents a lane's STAGE-GATE to the operator inline and relays the decision back to the owning orquestador (`agents/lider.md § Gate presentation protocol` — "Ask-class caveat"), the presentation is a request for a human decision, not a claim that anything is being mechanically "halted." The líder does not know the session's permission posture and must not imply a guarantee the `ask`-class gate does not make.
...
The ask-class caveat has a direct consequence for how STAGE-GATE-3 — the human push/PR gate — is surfaced and released in the lider+orquestador split.

**(a) The líder presents STAGE-GATE-3 inline and relays the operator's decision.** STAGE-GATE-3 is prepared and recorded inside the orquestador that owns the task; the líder presents its STOP block to the operator inline — in the operator's main conversation, the only reliably reachable channel — and relays the operator's decision (verbatim, tagged `lider-relayed-operator`) back to the orquestador, which records the release (`agents/lider.md § Gate presentation protocol`). Because the outward-action `ask` does not itself guarantee a stop (it can be auto-satisfied), the presentation must be an active, unmissable interactive surface that names three things: the orquestador (its slug), the gate (`STAGE-GATE-3`), and the decision the operator is being asked to make. A passive breadcrumb the operator might scroll past is insufficient — the human decision is the actual control here. The deterministic floor on the actual push/PR remains `dev-guard`'s native `ask`, not this presentation; the presentation is what routes the operator to that decision.

**(b) Anti-pattern: broad Bash auto-allow + lane mode.** Running a multi-lane fan-out under a blanket `Bash` auto-allow (or any posture that auto-answers `ask`) is an anti-pattern: it removes the human from the STAGE-GATE-3 outward-action prompt, so a lane's delivery push/merge could proceed without the operator ever entering the gate. The operator releases STAGE-GATE-3 by replying to the líder's inline presentation; the líder relays that decision to the owning orquestador, which records the release (the gate release travels operator → líder → orquestador, tagged `lider-relayed-operator`), and recover's STAGE-GATE-3 clear-allowlist requires `gate3_release = ship` (`skills/recover/SKILL.md § Rule 1`). The broad auto-allow posture defeats the interactive stop this design depends on.

**(c) recover's fail-safe covers gate NON-release, not an already-run `ask`-satisfied action.** `/th:recover`'s Rule 1 (re-present any un-cleared STAGE-GATE, fail-closed — `skills/recover/SKILL.md`) protects the case where a gate was never released: on resume it finds no `gate3_release = ship` plus `stage.gate.release` event and the orquestador returns its `gate_pending`, which `th:lider` re-presents inline. It does NOT and cannot undo an outward action that ALREADY RAN because an `ask` was auto-satisfied under a broad auto-allow — a push that already landed is not an un-cleared gate, it is a completed irreversible action with no state to re-present. The fail-safe is a forward re-prompt for un-taken decisions, never a rollback of a taken one. This is the residual the ask-class loosening acknowledges: the deterministic floor for the truly irreversible in-lane case is the `deny` in `policy-block` (see the ask-class caveat above), not recover.
...
- **Path-pattern auto-escalation (`lider.md § Phase 0a` classification):** sets `security_sensitive: true` based on file paths touched by the PR. This runs on the diff, not on the session state.
...
- `agents/lider.md` — intake, Discover phase, classification and routing.
- `agents/orquestador.md` — all phase contracts and gate enforcement.
