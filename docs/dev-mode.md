...
A limitation qualifies as "documented, not chased" only when it is BOTH (a) disclosed in-place where it lives, AND (b) scoped out through a legitimate mechanism: an architectural-inevitability limit for the string-matching-gate case, or an operator-approved out-of-scope decision plus a recommended follow-up issue for the classification-timing gap at `agents/orchestrator.md:1147`. The `orchestrator.md:1147` residual is a classification-scope gap — a DISTINCT class from string-matching obfuscation. Both are examples of the SAME disciplined-disclosure pattern, never the same residual class. Cross-refs: this file's "Residual static-resolution limits" section (`:39`) and `agents/orchestrator.md § "Cost-ordered re-run sequencing"` (`:1147`, the "Known, disclosed limitation" paragraph).
