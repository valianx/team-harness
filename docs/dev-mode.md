...
- **Security floors are PROMPT-INDEPENDENT (hooks, not prompt):** the security guarantees of this harness are PreToolUse hooks wired by matcher — they fire regardless of which system prompt is active. The enumerated catalogue (Bash-command gates + the new MCP-write gate) is:
  ...
  - `hooks/dev-guard.sh` — two dedicated PreToolUse entries: (a) `Bash`-only: gates outward/mutating Bash actions in dev mode (git push, gh pr merge/review/comment, gh api mutating PR endpoints, curl/wget to api.github.com; see § Outward-Action Gate); (b) `mcp__.*__clickup_(update_task|create_task|create_task_comment|attach_task_file)`: gates ClickUp MCP outward writes under the active marker — issues `ask` on any write, mirroring the git/gh catalogue backstop. Both entries survive the output-style swap intact.
...
**Session-init loads (dev-mode-independent):** `hooks/session-start.sh` is THE unified SessionStart loader. It runs three independent, fail-safe loads at every session start (regardless of dev mode) under the same `startup|resume|clear` matcher: (1) `load_dev_mode` — the marker-gated banner + disposition (this section); (2) `load_language` — the configured default language directive from `~/.claude/.team-harness.json`; (3) `load_workspace_mode` — the obsidian workspace base-path directive when `logs-mode: obsidian` is configured. All three emit into a single combined JSON; none-apply → emit nothing. An explicit per-session override from the operator takes precedence over the language directive for that session. The hook's header-comment REGISTRY block documents the ordered loads and the 3-step procedure for adding a future session-init load — new session-surfaced config goes there, not as a new SessionStart hook.
