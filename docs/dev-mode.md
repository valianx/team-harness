...
**The minimal floor.** The gate covers ONLY the irreversible publication boundary — the two hard points named at the end of this document (force-class pushes and PR merge) plus the default-branch push that is their common destination. Every other outward write — `gh pr create`, `gh pr review`, `gh pr comment`, `gh issue` writes, non-merge `gh api` mutations, MCP tool writes (ClickUp included) — is deliberately UNCOVERED: the hook emits no decision and the host runtime's own permission and approval model governs, configured directly by the operator. Rationale: those actions are reversible (a draft PR can be closed, a comment deleted), the host layer already prompts or allowlists them, and duplicating that control here was pure friction.
...
| `gh pr merge` | `ask` | Merging cannot be undone; merge stays human |
| `gh api` REST with a mutating method against `/pulls/{n}/merge` | `ask` | API-level bypass of `gh pr merge` |
| `gh api graphql` carrying the `mergePullRequest` mutation | `ask` | GraphQL-level bypass of `gh pr merge`; every other mutation name and all read-only queries stay ungated |
| Any non-covered call (Edit/Write/NotebookEdit payloads; benign Bash; all MCP tools) | no decision (exit 0, empty stdout) | Defer to the host runtime's permission flow |

Raw HTTP writes to `api.github.com` (`curl`/`wget` with a mutating method) are NOT a covered action — that gate was retired. The governing control is the prompt-level rule that agents never call the GitHub API directly: `git` and `gh` are the only sanctioned GitHub channels, with the documented `gh`-fallback (`agents/_shared/gh-fallback.md`, gh absent/unauthenticated) as the sole exception. A `gh api` merge call remains covered (table above) because it is the `gh` channel aimed at the one irreversible endpoint.

**What `ask` means:** `permissionDecision: "ask"` causes the Claude Code runtime to prompt the OPERATOR interactively for that specific call. The agent CANNOT auto-approve an `ask`. There is NO authorisation marker file — the authorisation is human out-of-band.

**Fail-CLOSED for covered actions:** the hook evaluates every covered action unconditionally and returns a destination-aware decision — `allow` only for the single recognized safe push form, `ask` for every other covered form (default-branch/tag/force/multi-refspec/non-`origin` pushes, merges). This is the intentional fail-mode: the consequences of an unauthorized merge to main are irreversible; the consequences of an over-zealous `ask` on the floor are a minor friction. Fail-closed applies WITHIN the covered set only — an uncovered action is not a fail-open, it is a deliberate delegation to the host layer.
...
- **A covered action buried in an interpreter's own program-text argument** (`awk 'BEGIN{system("git push origin main")}'`, `perl -e '...'`, `python -c '...'`, `sed 'e git push origin main'`). The token-granular forward-scan cannot see inside a single quoted program-text token — closing this requires parsing the interpreter's own grammar, the general de-obfuscation path this analyzer deliberately does not attempt. `policy-block` deliberately provides no raw-text git backstop; server-side branch protection is the real floor for this class.
...
**The line that decides.** A hook guards what is **irreversible** (a committed secret, a force-push) or **irreversibly outward-facing** (a merge, a destructive gcloud verb) — cases where no upstream contract can make the condition impossible, so the check at the choke point *is* the control. How work is done *inside* the pipeline is contract, not gate: an output contract the agent must satisfy, and tools scoped so the wrong action is unavailable rather than merely forbidden.
...
**Historical `gh pr create` coverage.** At the time this order floor was designed, `dev-guard` covered `gh pr create` (`ask` by default with an `allow` opt-in, `autogate.pr_create`). That coverage and the autogate config key were RETIRED with the minimal-floor recalibration — `gh pr create` is uncovered today (see the table above) — so references to it below are historical context for the retained body, not current behavior.

**What the retained `gate-guard` body modeled.** `gate-guard` (`hooks/ts/bodies/gate-guard.ts`) modeled whether a `git push` / `gh pr create` from a pipeline lane is preceded by a recorded `gate3_release: ship` for that lane. It resolves the governing lane by mtime-selecting the active `00-state.md` — parity with `checkpoint-guard`'s `selectByMtime` (local workspaces subtree +, when configured, the obsidian vault subtree) — then correlates the current git context against that lane. Correlation is branch-first in BOTH delivery topologies: when the lane declares a `working_branch` and the current branch is readable, branch equality alone decides — the lane owns exactly that branch, a match governs, and a mismatch defers to `dev-guard` (operator-directed non-pipeline work, e.g. the `/th:inline` posture, is never captured by an order gate it cannot satisfy). The worktree-realpath match (`realpath(cwd()) == realpath(worktree)`) is a fallback that governs only the lane's pre-branch window (no `working_branch` declared yet) or when the current branch cannot be resolved (fail-closed toward the lane); a branch-in-place lane (`worktree: null`) resolves by branch name alone. Accepted residual under the historical model: in-lane work pushed from a renamed branch escaped this correlation; registered force-push handling belongs to `dev-guard`. Full contract: `agents/_shared/gate-contract.md § "Outward-action release floor"`.
...
**Deny (`gate-guard`, ORDER) vs ask (`dev-guard`, destination) — independent and additive, not a replacement.** `gate-guard`'s decision set is `{none, deny}` only — it never emits `ask`, so it neither inherits nor removes the ask-class caveat below, which continues to apply unchanged to `dev-guard`'s own `ask` on `gh pr merge`. `dev-guard` gates by WHAT the command targets (destination), unconditionally on session state. `gate-guard` gates by WHETHER a release was recorded before this specific invocation (order), only when a pipeline lane resolves. Under the historical model, a push/pr-create from a detected lane had to clear BOTH checks independently — neither one substitutes for the other.
...
**Force-push behavior.** The retained but unwired `gate-guard` body denies a force-push from a detected pipeline lane in either flag or `+`-refspec form. In the registered runtime path, `dev-guard` is the single outward-action owner and routes every non-benign push shape, including force-push, to `ask`. `policy-block` deliberately owns no git workflow policy.
...
carries (`dev-guard`), not a regression introduced by this mechanism.

**Historical scope.** `gate-guard` retains its self-contained deny semantics so its historical body remains internally coherent, but it is not registered. The live path does not claim an in-lane hard deny: `dev-guard` asks on force-push and server-side branch protection remains authoritative.

**This design never touches or works around server-side branch protections.** Nothing here bypasses, disables, or reconfigures a repository's branch-protection rules. `gh api` merge writes remain `ask` under `dev-guard`, unchanged.

**The philosophy this design anchors: only two hard points.** Force-push (deny in-lane, `ask` outside) and merge (always `ask`, non-configurable) are the only two hard points in the outward-action model; every other git operation — branching, committing, pushing to a feature branch, opening a PR — stays frictionless. The minimal-floor recalibration made the table match this statement exactly: the previously covered `gh pr create/review/comment`, `gh issue` writes, non-merge `gh api` mutations, and ClickUp MCP writes are uncovered, governed by the host runtime's permission model. "Merge" in this statement means a **PR merge** — `gh pr merge`, or any action that lands commits on `main` or another protected branch — never a LOCAL `git merge origin/main` into the pipeline's own working branch (an ordinary fast-forward/update, which is unremarkable git handling and must never be asked or denied). This distinction holds at the classification level: the shared analyzer resolves a covered `gh pr merge` invocation to argv `["gh", "pr", "merge", …]` — lexically distinct from `git merge` — and `gate-guard` introduces no classification for any form of "merge" at all; a local `git merge` is not a covered action for either hook.
...
the human from the merge prompt — the one decision that always stays human. The Gate-3 release
authority originates in the operator's Gate-1 approval (`release_policy: auto-ship`); recover's
STAGE-GATE-3 clear-allowlist requires `gate3_release ∈ {ship, auto-ship}` with a valid citation
(`skills/recover/SKILL.md § Rule 1`). The broad auto-allow posture defeats the interactive stop
on the actions that remain gated.
...
| Coverage | `gh pr review`, `POST /reviews`, replies, dismiss | by DESTINATION: push to remote; `pulls/{n}/merge` via any binary | COMPLEMENTARY — review publication is prompt-governed; the hook floor covers only the irreversible merge/push boundary |

The gate does NOT re-implement the review-mode publish gate. Under the minimal floor, review/comment publication is governed by the #251/#252 prompt contract plus the host runtime's permission model; `dev-guard`'s hook floor covers the merge/push boundary those publications can never cross on their own. See `agents/ref-direct-modes.md § Publish Gate` for the review-mode contract.
