...
This document is the canonical contract. Every provisioning site in the codebase (`skills/setup/SKILL.md`, `agents/leader.md`) implements exactly this mechanism; do not introduce a variant mechanism at a new site.
...
| **B — Leader Phase 0a (existing-install / recurring)** | `agents/leader.md` § Phase 0a Step 1g | (a) `~/.claude/settings.json` for the obsidian workspace base — same destination as site A; (b) `.claude/settings.local.json` (project-local, gitignored) for cross-repo work-surface paths | (a) covers an install that never re-runs `/th:setup`, or a prior decline; (b) is per-pipeline, revertible | Every pipeline run, in obsidian mode (a) or when the pipeline declares an out-of-cwd work-surface path (b) |
