...
- **Author attribution on `[decision]` nodes.** Decisions are the highest-stakes type — readers need to know whose call it was. Promote `Decided by: <author-or-team>, <date>` to a required observation on every `[decision]` entry. The `session_start` / `session_end` tools provide the author proxy via `session_id`; orquestador Phase 6 attaches the session_id to the `create_nodes` call.
...
- ✅ Filter wired into `orquestador.md` Phase 6 (Knowledge Save).
...
This policy is **normative for humans and agents**. The orquestador's filter is the first line of defense at write time.
