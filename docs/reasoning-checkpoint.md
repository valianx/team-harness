...
- **B3 (post-verify → next).** Inside the orchestrator the post-verify transition is governed by the Phase 3.5 acceptance gate followed by the hard **STAGE-GATE-3** (a mandatory human gate), which subsumes B3; the orchestrator arms no separate `postverify-next` boundary.
