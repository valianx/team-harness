...
| `overview.md` (initiative) | operator-facing | `agents/ref-dispatch-machinery.md § "overview.md — you are the sole writer"` (unchanged) | orchestrator (sole writer; `delivery` only RETURNS per-project completion-row data, the coordinator reconciles and writes it) |
