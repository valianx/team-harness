...
assert.equal(
  ...
  "project config must leave native execution settings to the user",
);
...
assert.match(inlineReviewer, /^name = "inline-reviewer"$/m);
assert.match(inlineReviewer, /^# Semantic source: agents\/inline-reviewer\.md(?: \(|$)/m);
...
for (const role of ["implementer", "tester"]) {
  ...
  assert.ok(content.includes("## Specialist coordination"),
    `pipeline-${role} is missing the shared coordination adapter`);
}
...
assert.match(architect, /^# Semantic source: agents\/architect\.md /m);
assert.match(architect, /^sandbox_mode = "workspace-write"$/m);

for (const script of ["bounded-command.mjs", "cli-entrypoint.mjs", "code-hygiene.mjs", "workspace-identity.mjs", "herdr-message.mjs"]) {
...
for (const marker of ["Specialist coordination", "Native runtime permissions", "coordinator"]) {
...
for (const role of ["implementer", "tester"]) {
  ...
  assert.match(generatedAgent, /## Specialist coordination/, `${role} generated agent misses coordination adapter`);
  assert.match(generatedAgent, /Native runtime permissions/, `${role} generated agent misses native permission guidance`);
}
...
assert.match(roster, /`pipeline` when you choose broader coordination/);
...
assert.match(explicitPlanReview, /^name: plan-review$/m);
assert.match(explicitPlanReview, /Use an available plan reviewer/);
...
assert.match(pipelineContract, /^# Coordinated development workflow$/m);
assert.match(pipelineContract, /Native permissions govern/);
assert.match(pipelineContract, /Use `create-pr` to inspect the diff/);
const lintContract = await readFile(join(root, "skills/lint/SKILL.md"), "utf8");
assert.match(lintContract, /generated asset freshness/);
assert.doesNotMatch(lintContract, /native hooks/);
