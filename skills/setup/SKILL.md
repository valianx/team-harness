...
Why per-account config dirs: `gh` reads its credentials from `$GH_CONFIG_DIR/hosts.yml`. Pointing each account at its own directory lets a lane pin its identity by PATH (`GH_CONFIG_DIR={dir} gh --repo {owner/repo} …`) instead of relying on the single global `~/.config/gh/hosts.yml` whose "active account" drifts between runs (the diagnostic capture is documented in `agents/_shared/delivery-mechanics.md § 7`).
