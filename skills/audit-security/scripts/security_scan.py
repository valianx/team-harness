...
# Suite 12 — security self-scan (canonical copy; tests/test_security_scan.py
# is a thin wrapper that forwards to this file)
#
...
#
# Usage:
#   python3 skills/audit-security/scripts/security_scan.py [--root <asset-root>]
#   (or via the repo wrapper: python3 tests/test_security_scan.py)
# The asset root defaults to the nearest ancestor of this file containing an
# agents/ directory — the repo checkout, the CC plugin cache, the Codex
# package, and an opencode config root all satisfy that shape.
# Exit code:
...
def _resolve_asset_root() -> Path:
    ...
    if "--root" in argv:
        idx = argv.index("--root")
        if idx + 1 >= len(argv):
            sys.exit("asset root not found — pass --root <path>")
        root = Path(argv[idx + 1]).resolve()
        if not (root / "agents").is_dir():
            sys.exit(f"--root {root} has no agents/ directory")
        return root
...
def check_1_readonly_bash() -> int:
    ...
    for agent_name in READ_ONLY_AGENTS:
        ...
        if expected_tools is not None and agent_tools != expected_tools:
            # An opencode-projected agent (marked th-origin) carries a
            # deny-by-default permission block instead of a tools: field; the
            # exact tools allowlist is a source-tree contract, not a projected
            # one. The Bash/mutation checks above still apply.
            if "th-origin" in fm and "tools" not in fm:
                print(
                    f"  [SKIP] check-1 — agents/{agent_name}.md is an opencode"
                    " projection (th-origin, no tools:) — exact allowlist"
