...
Recover an interrupted pipeline or batch from where it left off. Routes to the **líder** (the top-level session agent) with full recovery context. The líder rebuilds tracking from `00-lider-roster.md` (roster-based; it never reads or writes any gate-release record — `agents/lider.md § lider-recover`); any un-cleared STAGE-GATE is re-prepared by the relevant **orquestador** when it resumes (it re-reads its own `00-state.md` dual-record and returns a `gate_pending` — `agents/orquestador.md § orquestador-recover`), which the líder re-presents to the operator inline. This skill records nothing and presents no gate itself.
...
**Rule 1 — Never bypass an un-cleared STAGE-GATE; the orquestador re-prepares it and the líder re-presents it (fail-closed).**
...
**Who prepares, who presents (present-vs-record split).** This skill is read-only: it runs the structural check above ONLY to surface which gate is un-cleared, so recovery routes to the right orquestador — it never records a release and presents no gate itself. The un-cleared determination is prepared inside that pipeline's own `th:orquestador`, which re-reads its own `00-state.md § Current State` dual-record on resume and returns a `gate_pending` for any un-cleared STAGE-GATE (`agents/orquestador.md § orquestador-recover`); the líder re-presents that `gate_pending` to the operator inline and relays the decision back for the orquestador to record. The líder rebuilds tracking from `00-lider-roster.md` without ever reading or writing a gate-release field — it presents and relays, but never records the release itself. The advisory `pending_gate` in the roster is a routing hint only — never a gate-clear signal.
...
5. Validate the state:
   ...
   - If `status: blocked-no-dispatch` → report:
     ```
     Pipeline '{feature}' hit a dispatch failure — the orquestador could not reach a subagent via Task.
...
6. Pass recovery context to the **líder** (the top-level session agent). The líder rebuilds tracking without reading or writing any gate-release record, then re-spawns / continues the relevant `th:orquestador`, which performs the record-based resume (re-reading its own `00-state.md` dual-record and returning a `gate_pending` for any un-cleared STAGE-GATE, which the líder re-presents inline):
   ```
   Recover Pipeline:
...
1. Check for `00-lider-roster.md` (the líder's durable index — `agents/lider.md § 00-lider-roster.md`) at the resolved base path or, for an initiative, the initiative root. This is the primary source: the líder re-launches an orquestador for any roster row still `RUNNING`/`FAILED` (`agents/lider.md § Multi-Task fan-out`). Fall back to the legacy `workspaces/batch-progress.md` when no roster is present.
2. If neither is found → "No batch tracking found. Use `/th:recover {feature}` for single pipeline recovery."
3. Read the roster (or `batch-progress.md`) — extract all tasks/lanes whose coarse `Status` is `RUNNING`/`FAILED` (roster) or `in_progress`/`iterating`/`blocked` (per-lane `00-state.md`). Read only the roster's coarse `Phase`/`Status`/`pending_gate` columns — never any orquestador gate-release field.
...
6. Report the batch state to the user:
   ...
   ```
7. Pass batch recovery context to the **líder** (the top-level session agent):
   ```
   Recover Batch:
...
The líder will (re-launching one `th:orquestador` per recovered lane — it never runs a pipeline itself):
...
- Track progress in `00-lider-roster.md` (and `batch-progress.md` where present) as normal

---

## Mode 3 — No input provided (`/th:recover`)

1. **Roster-first — render the líder→orquestador tree.** If `00-lider-roster.md` exists (resolved base path or initiative root), render it as the selection surface grouped by project — one row per orquestador with its coarse `Phase` / `Status` and the advisory `pending_gate` (read verbatim from the roster; never inferred from a gate-release field — the roster is a líder tracking index that carries no gate-release record):
   ```
...
- If the líder (or the orquestador it re-spawns) fails to recover → it will report the issue. The skill does not retry.

---

### Session-scoped override on recovery

When recovering a pipeline, the resolved override is re-applied from `00-state.md` § Current State — not re-parsed from chat. The resuming **orquestador** reads the override fields already stored in its own `00-state.md` and logs `operation.success` with detail `override re-applied from 00-state.md`.

If the operator re-states an override during recovery, the **líder** re-resolves it (session-override resolution is the líder's — `agents/lider.md § Session-scoped config override`) and propagates it into the resumed orquestador as a new session override for that run.

---

## Important

- **You read state. The líder and orquestador do NOT** — they receive the recovery context from you.
- Always route to the **líder** (the top-level session agent) — do NOT execute any pipeline yourself. The líder rebuilds tracking from the roster (never reading or writing a gate-release record) and re-spawns the relevant `th:orquestador`; that orquestador performs the record-based resume, returning a `gate_pending` for any un-cleared STAGE-GATE that the líder re-presents inline.
- The resuming orquestador uses the Recovery Instructions in its own `00-state.md` to know exactly what to do next, including returning its `gate_pending` for any un-cleared STAGE-GATE (which the líder re-presents to the operator inline).
- For batch recovery, the líder re-launches one orquestador worktree instance per lane — neither the líder nor this skill runs the pipelines itself.
