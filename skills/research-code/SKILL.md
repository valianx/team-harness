...
- Output: `workspaces/{topic-slug}/00-research.md` with hybrid evidence and a `## Code vs Docs Conflicts` section
...
| **Route** | Routes through the orchestrator; produces one consolidated `00-research.md` | Standalone skill; does NOT route through orchestrator; uses tmux fan-out |
| **Output** | One `00-research.md` (hybrid code + web evidence, conflict detection, gap-closure loop) | Per-repo audit reports; `00-consolidated.md`; profile/contract validation |
