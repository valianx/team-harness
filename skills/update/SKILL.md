...
- Division of labour with `/th:release`: `/th:release`'s post-tag Step 4 runs the same download/apply legs (catalog refresh + `claude plugin update` for Claude Code; the OS-appropriate updater for opencode, gated on the `VERSION` asset's publication) at release-cut time, but it does NOT sync the managed `~/.claude/CLAUDE.md` blocks — that sync stays exclusively `/th:update`'s contract, run standalone whenever the operator invokes it.
