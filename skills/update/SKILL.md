...
- This skill is for **plugin installations**. It refreshes the marketplace catalog, downloads the new version into the plugin cache (`claude plugin update`), reports the version delta, syncs the marker-delimited managed blocks in `~/.claude/CLAUDE.md` to the version being activated, and removes retired blocks (`dev-mode`, `nested-dispatch-takeover`, `dev-mode-entry`). It never edits repository files or writes operator configuration keys; it never touches `~/.claude/CLAUDE.md` content outside the managed-block markers, and never reloads the session — the operator activates the update with `/reload-plugins`.
...
- Division of labour with `/th:setup`: setup is the one-time bootstrap (workspace mode, native preferences, and the first write of the managed blocks); update is the repeatable command that keeps the catalog and the managed blocks in sync on every run. Re-running setup is never required as part of the update flow.
