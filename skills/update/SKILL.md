...
6a. **Provision the subagent-nesting-depth prerequisite (gated).** Full mechanism: `docs/setup-update-model.md § Architecture prerequisite: subagent nesting depth`. This step applies only the concrete values below — it does not restate the mechanism.
   ...
   - This step is advisory-blocking only in the sense of its own Y/n — it never blocks the rest of the update flow; a decline, an aborted corrupted-file write, or a failed write simply leaves the depth setting unprovisioned — a specialist leaf agent invoked one level deep (a skill wrapper, an `@`-mention inside an ongoing session) may not retain the tools its own contract grants (`docs/subagent-orchestration.md § "Nested-context dispatch — RETIRED protocol, retained provisioning"`).
