...
- **Routes to th-orchestrator** (default for pipeline work): `/th:issue`, `/th:plan`, `/th:design`, `/th:research`, `/th:spike`, `/th:test`, `/th:test-pipeline`, `/th:validate`, `/th:define-ac`, `/th:security`, `/th:audit`, `/th:review-pr`, `/th:deliver`, `/th:diagram`, `/th:likec4-diagram`, `/th:d2-diagram`, `/th:translate`, `/th:init`, `/th:recover`, `/th:eval`, `/th:gcp-costs`, `/th:cross-repo`.
- **Standalone** (no th-orchestrator involvement): `/th:lint`, `/th:status`, `/th:memory`, `/th:tmux`, `/th:background`, `/th:update`.
...
This contract is universal across every routing skill. The full directive lives in `agents/th-orchestrator.md` § "Dispatch-blocked exit" and in `CLAUDE.md` § 13 "Subagent Orchestration" — both are authoritative.
