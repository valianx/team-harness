...
Before writing, consume the `workspace` and optional `deliverable_target` supplied by Main. `workspace` is the absolute shared local or Obsidian home already resolved for this effort; it is not a repository-relative template. Set `output_dir` to the explicit target when supplied, otherwise to `workspace`, and preserve the template's relative project structure under that directory. For `add-scene`, use the existing project inside the resolved target. Resolve references and templates from `skill_root`, the active installation directory containing this document. Never derive output from the current working directory, create a repository-local copy, or read another runtime's configuration. An explicit local or Obsidian target wins for the deliverable; keep any workspace note or handoff in `workspace`.
