...
- **`/th:background` does NOT invoke the lider.** This is a deliberately different surface — the orquestador and its gates exist precisely because most tasks are not eligible for fast-path. If you find yourself wanting to bypass the gates often, the cost is the gates being too heavy, not the gates being wrong; raise it as a `team-harness` issue instead of widening `/th:background`'s eligibility criteria.
