...
When the task enters an explicitly activated pipeline, retain its ID and URL as source context
in the bound OpenSpec proposal. Recovery and delivery read that canonical context; Main never
adds task-reference fields to generated `00-state.md`. Posting the closing comment still
requires the live approval specified below.
