...
This is the Codex projection of the canonical Team Harness `test-pipeline`
...
1. Treat the live operator request for `$team-harness:test-pipeline` as the canonical
   `$ARGUMENTS` value. Examples use `$team-harness:test-pipeline` as the supported Codex skill invocation; do not try to execute it as a shell command.
