...
This is the Codex projection of the canonical Team Harness `report-issue`
...
1. Treat the live operator request for `$team-harness:report-issue` as the canonical
   `$ARGUMENTS` value. Examples use `$team-harness:report-issue` as the supported Codex skill invocation; do not try to execute it as a shell command.
