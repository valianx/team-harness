...
This is the Codex projection of the canonical Team Harness `mcp-optimize`
...
1. Treat the live operator request for `$team-harness:mcp-optimize` as the canonical
   `$ARGUMENTS` value. Examples use `$team-harness:mcp-optimize` as the supported Codex skill invocation; do not try to execute it as a shell command.
