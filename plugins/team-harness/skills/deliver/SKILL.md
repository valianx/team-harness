...
the recorded `freeze_commit_sha`/`freeze_tree_sha` accepted by validation. That live reply is the
...
`unknown` without mutating refs or the accepted Freeze commit. Native Codex tool approval may still be surfaced as a technical
