...
This is the Codex projection of the canonical Team Harness `test-cross-browser`
...
1. Treat the live operator request for `$team-harness:test-cross-browser` as the canonical
   `$ARGUMENTS` value. Examples use `$team-harness:test-cross-browser` as the supported Codex skill invocation; do not try to execute it as a shell command.
