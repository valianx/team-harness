...
This is the Codex projection of the canonical Team Harness `json-canvas`
...
1. Treat the live operator request for `$team-harness:json-canvas` as the canonical
   `$ARGUMENTS` value. Examples use `$team-harness:json-canvas` as the supported Codex skill invocation; do not try to execute it as a shell command.
