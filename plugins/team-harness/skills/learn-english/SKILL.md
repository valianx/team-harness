...
This is the Codex projection of the canonical Team Harness `learn-english`
...
1. Treat the live operator request for `$team-harness:learn-english` as the canonical
   `$ARGUMENTS` value. Examples use `$team-harness:learn-english` as the supported Codex skill invocation; do not try to execute it as a shell command.
...
6. Read and update only the Codex-native Team Harness configuration through its packaged configuration helper.
