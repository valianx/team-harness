...
This is the Codex projection of the canonical Team Harness `find-bugs`
...
1. Treat the live operator request for `$team-harness:find-bugs` as the canonical
   `$ARGUMENTS` value. Examples written as `/th:find-bugs` name the same capability; do not try to execute them as shell commands.
