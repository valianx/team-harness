...
This is the Codex projection of the canonical Team Harness `background`
...
1. Treat the live operator request for `$team-harness:background` as the canonical
   `$ARGUMENTS` value. Examples written as `/th:background` name the same capability; do not try to execute them as shell commands.
...
6. Use Codex subagents for bounded background work. Never launch or require the `claude` binary.
