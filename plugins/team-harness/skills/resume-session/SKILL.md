...
This is the Codex projection of the canonical Team Harness `resume-session`
...
1. Treat the live operator request for `$team-harness:resume-session` as the canonical
   `$ARGUMENTS` value. Examples use `$team-harness:resume-session` as the supported Codex skill invocation; do not try to execute it as a shell command.
...
6. Resolve workspace settings from the Codex-native Team Harness configuration and remain strictly read-only.
