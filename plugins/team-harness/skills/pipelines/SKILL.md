...
This is the Codex projection of the canonical Team Harness `pipelines`
...
1. Treat the live operator request for `$team-harness:pipelines` as the canonical
   `$ARGUMENTS` value. Examples written as `/th:pipelines` name the same capability; do not try to execute them as shell commands.
...
6. Resolve workspace settings from the Codex-native Team Harness configuration and render state read-only.
