...
This is the Codex projection of the canonical Team Harness `audit-security`
...
1. Treat the live operator request for `$team-harness:audit-security` as the canonical
   `$ARGUMENTS` value. Examples use `$team-harness:audit-security` as the supported Codex skill invocation; do not try to execute it as a shell command.
