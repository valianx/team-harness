...
This is the Codex projection of the canonical Team Harness `kg`
...
1. Treat the live operator request for `$team-harness:kg` as the canonical
   `$ARGUMENTS` value. Examples use `$team-harness:kg` as the supported Codex skill invocation; do not try to execute it as a shell command.
...
6. Use the configured Memory MCP when available; otherwise report the missing native dependency without substituting another store.
