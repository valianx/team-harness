...
This is the Codex projection of the canonical Team Harness `bootstrap`
...
1. Treat the live operator request for `$team-harness:bootstrap` as the canonical
   `$ARGUMENTS` value. Examples use `$team-harness:bootstrap` as the supported Codex skill invocation; do not try to execute it as a shell command.
