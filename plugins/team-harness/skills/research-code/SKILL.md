...
This is the Codex projection of the canonical Team Harness `research-code`
...
1. Treat the live operator request for `$team-harness:research-code` as the canonical
   `$ARGUMENTS` value. Examples use `$team-harness:research-code` as the supported Codex skill invocation; do not try to execute it as a shell command.
...
6. Use Codex explorers or other bounded subagents only when independent lanes are useful and allowed; Main consolidates all evidence.
