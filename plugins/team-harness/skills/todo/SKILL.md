...
This is the Codex projection of the canonical Team Harness `todo`
...
1. Treat the live operator request for `$team-harness:todo` as the canonical
   `$ARGUMENTS` value. Examples use `$team-harness:todo` as the supported Codex skill invocation; do not try to execute it as a shell command.
...
6. Store persistent values in the Codex-native Team Harness configuration and preserve unrelated keys.
