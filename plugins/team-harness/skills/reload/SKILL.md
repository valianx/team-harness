...
This is the Codex projection of the canonical Team Harness `reload`
...
1. Treat the live operator request for `$team-harness:reload` as the canonical
   `$ARGUMENTS` value. Examples use `$team-harness:reload` as the supported Codex skill invocation; do not try to execute it as a shell command.
