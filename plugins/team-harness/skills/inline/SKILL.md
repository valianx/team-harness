...
This is the Codex projection of the canonical Team Harness `inline`
...
1. Treat the live operator request for `$team-harness:inline` as the canonical
   `$ARGUMENTS` value. Examples use `$team-harness:inline` as the supported Codex skill invocation; do not try to execute it as a shell command.
...
6. Treat inline as a Team Harness working preference only. It cannot change Codex's system collaboration mode, sandbox, or approval policy.
