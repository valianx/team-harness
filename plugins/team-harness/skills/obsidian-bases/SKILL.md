...
This is the Codex projection of the canonical Team Harness `obsidian-bases`
...
1. Treat the live operator request for `$team-harness:obsidian-bases` as the canonical
   `$ARGUMENTS` value. Examples use `$team-harness:obsidian-bases` as the supported Codex skill invocation; do not try to execute it as a shell command.
...
6. Resolve vault settings from Codex-native Team Harness configuration or explicit operator input, never from a Claude-only config path.
