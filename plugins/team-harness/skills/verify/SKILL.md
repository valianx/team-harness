...
This is the Codex projection of the canonical Team Harness `verify`
...
1. Treat the live operator request for `$team-harness:verify` as the canonical
   `$ARGUMENTS` value. Examples use `$team-harness:verify` as the supported Codex skill invocation; do not try to execute it as a shell command.
