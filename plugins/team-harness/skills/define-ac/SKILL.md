...
This is the Codex projection of the canonical Team Harness `define-ac`
...
1. Treat the live operator request for `$team-harness:define-ac` as the canonical
   `$ARGUMENTS` value. Examples use `$team-harness:define-ac` as the supported Codex skill invocation; do not try to execute it as a shell command.
