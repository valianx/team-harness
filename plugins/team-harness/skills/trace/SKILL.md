...
This is the Codex projection of the canonical Team Harness `trace`
...
1. Treat the live operator request for `$team-harness:trace` as the canonical
   `$ARGUMENTS` value. Examples use `$team-harness:trace` as the supported Codex skill invocation; do not try to execute it as a shell command.
...
6. Resolve workspace and pricing settings from the Codex-native Team Harness configuration and remain read-only.
