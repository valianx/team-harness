...
This is the Codex projection of the canonical Team Harness `clickup`
...
1. Treat the live operator request for `$team-harness:clickup` as the canonical
   `$ARGUMENTS` value. Examples use `$team-harness:clickup` as the supported Codex skill invocation; do not try to execute it as a shell command.
...
6. Store persistent values in the Codex-native Team Harness configuration. Preserve all opaque keys and never print credentials.
