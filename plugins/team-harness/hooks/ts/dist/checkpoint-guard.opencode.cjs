...
var checkpoint_guard_opencode_exports = {};
__export(checkpoint_guard_opencode_exports, {
  default: () => checkpointGuardPlugin
});
module.exports = __toCommonJS(checkpoint_guard_opencode_exports);
  ...
  return {
      ...
      try {
        ...
        while (stack.length > 0) {
          ...
          for (const e of entries) {
            const fullPath = path2.join(current, e.name);
            if (e.isDirectory()) {
              stack.push(fullPath);
            } else if (e.name === name) {
    ...
    gitRepoName() {
      try {
        ...
        if (!out) return null;
        const absCommonDir = path2.isAbsolute(out) ? out : path2.resolve(cwd, out);
        const repoRoot = path2.dirname(absCommonDir);
...
function checkpointGuardPlugin() {
  return {
    hooks: {
