...
var prepublish_guard_opencode_exports = {};
__export(prepublish_guard_opencode_exports, {
  default: () => prepublishGuardPlugin
});
module.exports = __toCommonJS(prepublish_guard_opencode_exports);
...
function inboundOpencode(input, _output) {
  ...
  const mapped = {
    ...
    tool: typeof toolName === "string" ? {
      name: toolName,
      input: typeof toolArgs === "object" && toolArgs !== null && !Array.isArray(toolArgs) ? toolArgs : {}
    } : null,
...
function isShippedPath(path3) {
  return SHIPPED_PATH_RE.test(path3) || SHIPPED_FILE_RE.test(path3) || INSTALLER_SOURCE_RE.test(path3);
}
...
function resolveOpencodeConfigRoot() {
  ...
  return {
    ...
    readConfig() {
      try {
        ...
        const configPath = path2.join(configRoot, ".team-harness.json");
...
function resolveWorktreeCwd(input) {
...
function prepublishGuardPlugin() {
  return {
    hooks: {
