...
One agent owns coordination. **`orchestrator`** is the top-level session agent and the operator's single point of contact. Its startup kernel serves direct work by default; `/th:pipeline` explicitly activates the lazy-loaded v5 contract in `ref-pipeline.md`. During an active pipeline it owns intake, OpenSpec binding, workspace/control state, both STAGE-GATEs, and every control-log append and projection. It never dispatches another coordinator.
...
entry requires explicit live activation or recovery and always uses the canonical v5 machine.
...
| `adversary` | Try to break every changed security control in the frozen delivery diff and return `broke-it` or `could-not-break`; never certifies. | sonnet | `xhigh` | Read, Glob, Grep, Edit, Write | Independent adversarial lens for `/th:verify` and the spec lane's in-lane security path; the v5 pipeline does not dispatch it. **No Bash or external search.** Writes only its own English report; `Edit` supports recovery and amend reports without touching source. |
...
| `qa` | Independently verify a frozen candidate against canonical acceptance and quality evidence; never writes code or planning content. | opus | `xhigh` | Read, Glob, Grep, Edit, Write, `mcp__memory__search_nodes`, `mcp__memory__open_nodes` | The one fresh ordinary verifier over the frozen implementation and OpenSpec evidence; also docs-validation and cross-repo review. **No Bash** (read-only on system). KG read-only for AC-pattern lookup. |
| `plan-reviewer` | Audit canonical OpenSpec and compact `01-plan.md` projection fidelity when explicitly requested; never modifies either source. | sonnet | `medium` | Read, Glob, Grep | Single read-only audit for `/th:plan-review`; no automatic pre-gate dispatch or report write. |
...
| `gcp-infra` | Plan GCP infrastructure changes via generated `gcloud` scripts; execute only the concrete plan authorized by the operator. | opus | `xhigh` | Read, Bash, Glob, Grep, Write | Inspect, plan, validate and independently review GCP changes. Reuse clear authorization for unchanged effects, including disclosed destructive consequences; native permissions govern apply. |
...
| `implementer` | Write the smallest approved production diff and ordinary owned tests; never designs architecture. | sonnet | `high` | Read, Edit, Write, Bash, Glob, Grep, NotebookEdit | Production code, ordinary tests, and narrowly scoped canonical documentation following pinned OpenSpec. |
...
- `_shared/ac-evidence.md` — canonical acceptance evidence, independent-test predicate, and verifier/security freshness contract. Consumed by `tester.md`, `qa.md`, and `ref-pipeline.md`.
...
**Tally (standard mode):** core `opus` agents — `orchestrator`, architect, qa, agent-builder, security, mentor (plus the GCP/UX specialists); `haiku` — `researcher`, `init-project`; everything else on `sonnet`, including `adversary`, `reviewer`, `reviewer-consolidator`, and `translator`. In low-cost mode, all on `sonnet`. No `max`, no `low`.
