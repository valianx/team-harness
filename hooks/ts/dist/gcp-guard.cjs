...
var READ_ONLY_RE = /^(list|describe|search-all-resources|simulator|replay-recent-access|recommendations|get|get-.+|print-.+)$/;
var DESTRUCTIVE_RE = /^(delete|remove-.+|remove-iam-policy-binding|purge|clear-.+|destroy)$/;
var MUTATING_RE = /^(create|update|patch|add-.+|set-.+|enable|disable|resize|start|stop|deploy|import|add-iam-policy-binding|set-iam-policy)$/;
...
function classifySegment(segment) {
  ...
  if (CATASTROPHIC_PROJECTS_RE.test(segment)) {
    segClass = 4;
    segVerb = "projects delete";
  } else if (CATASTROPHIC_FOLDERS_RE.test(segment)) {
    segClass = 4;
    segVerb = "resource-manager folders delete";
  } else if (CATASTROPHIC_ORGS_RE.test(segment)) {
    segClass = 4;
    segVerb = "organizations delete";
