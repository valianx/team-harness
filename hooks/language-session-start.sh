#!/usr/bin/env bash
# language-session-start.sh — Team Harness SessionStart hook.
#
# Reads the configured default language from ~/.claude/.team-harness.json and
# injects a one-time additionalContext directive instructing the agent to respond
# in that language for the whole session.
#
# Runs at every session start (startup|resume|clear), independent of dev mode.
# Coexists with dev-mode-session-start.sh — both run under the same matcher;
# each emits its own output; order is independent.
#
# Fail-safe: any read error, missing file, missing key, or invalid value → emit
# nothing, exit 0. Session start is never blocked.
#
