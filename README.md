...
> Team Harness is a multi-agent orchestration system for **Claude Code**: the top-level coordinator **`th:lider`** frames each request and spawns a **`th:orquestador`** per task, which dispatches specialized architect, implementer, tester, QA, security, and delivery agents through a Spec-Driven Development (SDD) pipeline with mandatory human gates.
>
...
After install, open Claude Code. The top-level session agent is **`th:lider`** — the operator's single point of contact. You drive the pipeline by talking to it directly (no `@mention` needed); `th:lider` frames the request and spawns a `th:orquestador` per task to run the gated execution pipeline. The entry points are:

- `th:lider` — the top-level session agent; just describe what you want
...
| [Agent tree](./docs/agent-tree.md) | How `th:lider`, `th:orquestador`, and the leaf specialists relate at runtime |
