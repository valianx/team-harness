...
def check_pr_review_workspace_isolation() -> None:
    ...
    require('git --git-dir "$SNAPSHOT_GIT" diff' in canonical, "PR review diff does not use the isolated snapshot repository")
    require('git --git-dir "$SNAPSHOT_GIT" worktree add --detach' in canonical, "PR review worktree administration still targets the operator checkout")
    ...
    require("complete capture is bounded to 60 seconds" in flattened, "PR review capture is not explicitly bounded")
