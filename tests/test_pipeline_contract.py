...
def check_ad_hoc_review_boundary() -> None:
    ...
    for marker in ("coordinates", "scope", "intent", "criteria", "changed_surface", "requested_lenses", "required_lenses"):
    ...
    for marker in ("installed native profile", "tester|qa|security|adversary", "non-empty", "unique", "security floor", "security_floor", "dispatch_id", "expected_lens", "coordinates", "blocker/high/medium", "unresolved blocking disagreement"):
...
def check_inline_markers(contract: str) -> None:
    markers = ("mode: inline-review", "repository_root", "commit_or_range", "scope", "intent", "criteria", "changed_surface", "requested_lenses", "required_lenses", "lens: tester|qa|security|adversary", "closed", "non-empty", "unique", "expected_lens", "dispatch_id", "security_floor", "target_id", "coordinates", "installed native profile", "sandbox_mode = \"read-only\"", "security floor", "authentication", "authorization", "ambiguity is sensitive", "lens_status: complete|incomplete|failed|unavailable|untrusted", "verdict: pass", "blocker/high/medium", "unresolved blocking disagreement", "never averages verdicts", "absence", "review-pr")
    contract = re.sub(r"\s+", " ", contract.lower())
    for marker in markers:
        require(marker in contract, f"inline contract missing {marker!r}")
    for marker in ("filesystem-root confinement", "profile digest", "session attestation", "restart requirement", "native runtime permissions and approvals remain authoritative"):
        require(marker in contract, f"inline native boundary missing {marker!r}")
    for marker in ("every required lens", "blocker/high/medium", "unresolved blocking disagreement", "never averages verdicts", "absence", "verdict: pass"):
...
def check_inline_native_boundary() -> None:
    """Inline review relies on the installed native profile without extra isolation."""
    contract = re.sub(r"\s+", " ", read("agents/_shared/inline-review-contract.md").lower())
    for marker in (
        "installed native profile",
        "sandbox_mode = \"read-only\"",
        "native runtime permissions and approvals remain authoritative",
        "filesystem-root confinement",
        "profile digest",
        "session attestation",
        "restart requirement",
    ):
        require(marker in contract, f"inline native-boundary marker missing: {marker!r}")
    for retired in (
        "profile_session",
        "git_optional_locks",
        "git_no_lazy_fetch",
        ...
        "cat-file blob",
        "regular non-symlink",
        "sha-256 raw-byte digest",
    ):
        require(retired not in contract, f"inline contract retains extra isolation protocol: {retired!r}")
...
def check_inline_review_contract() -> None:
    ...
    for label, text in (("Claude inline-reviewer", reviewer), ("Codex inline-reviewer", adapter)):
        for marker in ("tester", "qa", "security", "adversary", "repository_root", "commit_or_range", "native", "target_id", "coordinates", "expected_lens", "dispatch_id", "coverage", "disagreements", "blocker", "high", "medium"):
    ...
    for marker in ("inline-reviewer", "sandbox_mode = \"read-only\"", "adversary", "security floor", "dispatch_id", "expected_lens", "native runtime permissions and approvals"):
...
def main() -> None:
    checks = (
        ...
        ("inline native boundary", check_inline_native_boundary),
