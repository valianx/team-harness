...
- **The th-orchestrator pipeline.** Phase 2.5 / 4.5 only fire inside a real pipeline run. To smoke-test, run a feature through `/issue` or a plain feature description and check that `00-execution-events.jsonl`, `done.yml`, and `04-internal-review.md` appear in `session-docs/{feature}/`.
