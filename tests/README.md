...
- **The orchestrator pipeline.** Phase 2.5 (Constraint Reconciliation) only fires inside a real pipeline run. To smoke-test, run a feature through `/issue` or a plain feature description and check that BOTH `00-execution-events.jsonl` (local mode) or `00-execution-events.md` (obsidian mode) AND `00-pipeline-summary.md` appear in `workspaces/{feature}/` — both are mandatory for any non-Tier-0 run, and checking only the event log passes while a missing summary goes unnoticed.
