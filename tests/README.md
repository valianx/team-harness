...
- **The orchestrator pipeline.** Phase 2.5 (Constraint Reconciliation) only fires inside a real pipeline run. To smoke-test, run a feature through `/issue` or a plain feature description and check that `00-execution-events.jsonl` (local mode) or `00-execution-events.md` (obsidian mode) appears in `workspaces/{feature}/`.
