...
await check("a bounded cleanup preserves behavior and improves CRAP before Freeze", async () => {
  await repository(async (context) => {
    ...
    reordered.quality.commands.reverse();
    assert.equal(isCleanerTransitionResult(reordered), true);
...
await check("cleaner checks do not execute failing probes from unselected commands", async () => {
  await repository(async (context) => {
    const quality = manifest({ crap: false });
    quality.commands.database = command("process.exit(0);");
    quality.commands.database.version_argv = [node, "-e", "process.exit(23);"];
...
await check("post-check rejects added, deleted, or out-of-allowlist paths", async () => {
  await repository(async (context) => {
    const pre = await runCleanerTransition(preOptions(context));
    const evidence = await persistBaseline(context, pre);
    await rm(path.join(context.repo, "src/feature.js"));
...
await check("non-repository inputs and missing safety contracts use specific errors", async () => {
  ...
  try {
    const nonRepository = await runCleanerTransition({
      ...
      base: "HEAD",
      candidate: "HEAD",
      allowlist: path.join(directory, "allowlist.json"),
    });
    assertClosedResult(nonRepository);
    assert.equal(nonRepository.error_code, "ARGUMENT_INVALID");
  } finally {
