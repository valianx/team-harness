...
1. **Hooks fire at subagent depth ≥2.** A specialist dispatched by an orquestador subagent (itself a subagent of the líder) is a depth-≥2 node. `policy-block` and `dev-guard` (PreToolUse) must still fire for that node's tool calls — not only for depth-0/depth-1 calls.
...
|---|---|---|---|---|
| M2.1 | `policy-block` Write DENY fires at subagent depth (Write, secret pattern) | Part B (live) | **CONFIRMED — spike-sourced** | `research/spike-probe-evidence.md` §1 ("Write de secreto `ghp_`+36, subagente → BLOQUEADO"). Observed at depth ≥1 (spike ran a single-level nested subagent, not the specific líder→orquestador→specialist depth-2 shape the split introduces). |
...
Part A (decision-function correctness) is fully closed by deterministic tests authored in Task-6 — no live run needed for that half. Part B (does the harness actually route depth-2 hook invocations with a payload cwd that matches the nested subagent's real shell cwd) is **REQUIRES-LIVE-RUN**: the spike gives strong prior confidence (hooks fire at depth ≥1 for both Write and Bash, and the decision logic is separately verified correct), but the exact depth-2 líder→orquestador→specialist shape has not been directly observed. This is the item Task-9 registers as `probe_result` only after operator confirmation.
...
- Depth counting convention: this probe treats the líder (top-level, never a subagent) as depth 0, the orquestador it spawns as depth 1, and a specialist that orquestador dispatches as depth 2 — consistent with `01-plan.md`'s "profundidad ≥2" framing for AC-7.1.
