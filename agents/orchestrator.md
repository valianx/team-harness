...
| `adversary` | Independent adversarial reviewer with a break-the-design mandate; runs in Stage-2 verify in parallel with `security` when `adversary_floor_applies == true` (a narrower subset of `security_floor_applies`); verdict `broke-it \| could-not-break`; report in English | No | `reviews/04-adversary-r{N}.md` (per-round) |
...
{docs_root}/
  ...
  reviews/04-adversary-r{N}.md ← adversary (per-round; only if adversary_floor_applies AND tier/type predicate)
...
- adversary_rounds: {N | null}               # authoritative per-round adversary-report counter (R2/SEC-DR-F4) — you declare this BEFORE every adversary dispatch (initial Phase-3, Case-D re-dispatch, staleness re-gate, or an integrity-scan-triggered retry); strictly monotonic-increasing per dispatch, never reused on a retry of a failed round; the report-integrity scan and `delivery` both read this field as authoritative, never a filesystem-max glob over `reviews/04-adversary-r*.md`
- adversary_round_sizes: {{N: bytes, ...} | null}  # per-round byte-size baseline (§ "Adversary per-round report-integrity scan" check 3) — you record `adversary_round_sizes[N]` = the current byte size of `reviews/04-adversary-r{N}.md` as part of the SAME Phase Transition Protocol write that accepts round N's verdict; the next round's check 3 (no prior-round shrink) reads this map as its baseline, never an in-memory-only value that a compaction or `/th:recover` would lose
- adversary_consecutive_broke_it: {N | 0}     # whack-a-mole Variant-2 counter (§ "Whack-a-mole structural escalation" below, canonical: `docs/patch-mode.md § Whack-a-Mole Structural Escalation`) — increments by 1 per integrity-clean `adversary` round returning `broke-it` on a changed/hard-point control, resets to 0 on a `could-not-break` verdict AND on every fresh base pass that follows a structural redesign (the counter is design-model-scoped, never task-scoped)
- structural_escalations: {N | 0}             # whack-a-mole architect-redesign pivot counter (same section) — increments by 1 each time the detector triggers a redesign; cap = 1 by default, a 2nd trigger escalates to the operator; MONOTONIC per task, NEVER reset by a redesign or a fresh base pass (only the implementer iteration budget above resets)
- whack_a_mole_episode_start_round: {N | null} # whack-a-mole episode-boundary bookkeeping field (§ "Whack-a-mole structural escalation" below) — set to the current `adversary_rounds` value the FIRST time `adversary` is dispatched for this task's current episode, via EITHER of two reachable paths: (a) a task's Phase-3 base pass dispatches `adversary` (the ordinary case — the episode start is the base-pass round); OR (b) a task whose base pass did NOT dispatch `adversary` (cached `adversary_floor_applies: false` at that time) has it first dispatched mid-iteration via the limb-(b) recompute-and-recache extension (§ "Recompute-and-recache trigger extended to this branch" above) — the episode start is THAT dispatch's round, since no base-pass round exists to anchor to on this path; reset to `null` when a fresh base pass runs after a detector-triggered redesign (a new episode begins); read by § "Action on trigger" step 3 to derive the accumulated break-set's round range (this field through the triggering round), never reconstructed from unstructured `00-state.md` history or filesystem enumeration. **Never left `null` while an adversary round series exists for this task's current episode** — whichever path first dispatches `adversary`, this field MUST be set at that dispatch.
- changes_security_control: {true|false|null} # architect-declared Classification-block boolean (`agents/architect.md § Classification block`) — mirrored here at Design time, and re-written at any Phase-2-close/Phase-1.5-precheck force-set site (§ "Unified rule governing every `security_sensitive`-force-set site") or Case-B correction (§ "Security-verdict staleness re-gate"); absence or doubt reads as fail-closed `true`, per its own producer-site default — never left unmirrored here, or recovery/delivery cannot determine adversary-gate eligibility without re-deriving it
- adversary_floor_applies: {true|false|null}  # cached, orchestrator-computed value of `security_floor_applies AND changes_security_control` (§ "Adversary floor predicate") — you write this field every time you (re)compute or recompute the predicate (initial Phase 3 dispatch, and the staleness re-gate's "Recompute-and-recache" step); `delivery` and any recovery read this cached value verbatim and never re-derive it locally (SEC-DR-F6) — an absent value here is the same recovery/handoff gap this field exists to close
...
========================================
 ...
 {SEC-002 design-review verdict + Phase-3 security/adversary verdicts — omitted entirely when security_sensitive: false}
...
**Security never carried forward on a security-surface touch (fail-safe, non-negotiable).** A `security` sub-verdict is NEVER carried forward when the correction touched the security-relevant surface (bucket 2) — bucket 2 always forces a fresh `security` run. This is the Stage-1 analog of the existing Phase-3 "Security-verdict staleness re-gate" (`agents/orchestrator.md § If any agent fails → ITERATE`). When in doubt whether a correction touches the security-relevant surface, classify it as bucket 2 (or route to the full panel per the fail-safe rule above) — never assume non-security and carry the `security` sub-verdict forward on doubt.
...
**Unified rule governing every `security_sensitive`-force-set site in this file (stated once, binding on both branches below, on the Phase 1.5 T2-AC-17 pre-check above, and on any additional force-set site added anywhere in this file in the future).** Every site in this file that force-sets `security_sensitive: true` — by whatever trigger, at whatever phase — also force-sets `changes_security_control: true` on the same terms: unless `changes_security_control` was already explicitly and freshly declared `true` by the architect for this task, force-set it to `true` for the remainder of the task, fail-closed, per the same discipline `changes_security_control`'s own default already applies on doubt or absence (`agents/architect.md § Classification block`). Where this makes `adversary_floor_applies` (`= security_floor_applies AND changes_security_control`) evaluate `true`, it does so as a direct, named consequence of the firing site — never an inline re-derivation — so Phase 3 dispatches BOTH `security` and `adversary` by named-predicate discipline (SEC-DR-F6, T1-AC-9), not by an unconditional "ensure both run" instruction disconnected from the predicates.

**Exhaustive site enumeration (as of this audit — three force-set sites, four branches, file-wide).** (1) The Phase 1.5 T2-AC-17 pre-check's match branch (§ "Pre-check — path-pattern sensitivity recheck", above). (2) That same pre-check's own fail-closed-on-ambiguity branch. (3) This backstop's confirmed-match branch, immediately below. (4) This backstop's own fail-closed-on-ambiguity branch, further below. This rule is invoked BY NAME and restated locally at each of the four branches — never assumed by proximity to this paragraph — precisely because a rule previously scoped only to "this backstop" is what let site (1)/(2) go unpaired and unexamined for three consecutive rounds of adversarial audit, entirely outside the section this paragraph used to claim exhaustive coverage of. A future edit that adds a fifth force-set branch anywhere in this file must either apply this same pairing or explicitly document, at that new branch, why it is exempt — silence is the defect this rule exists to close, not either resolution per se.

On any match — path-pattern OR content-trigger — where `security_sensitive` is not already `true` in this task's `00-state.md § Current State`, force-set it to `true` for the remainder of the task, applying the unified rule above (also force-set `changes_security_control: true` on the same terms). This guarantees `security_floor_applies` evaluates `true` at Phase 3 (§ "Single shared Phase-3 floor predicate", T2-AC-10). **A backstop firing at all is itself evidence the earlier classification was wrong** — the same classification-blind-spot scenario the design's Security Assessment names as highest-risk.

**Scope boundary of this branch's pairing, stated honestly (a documented limitation, not a defect left open).** The "where `security_sensitive` is not already `true`" qualifier two sentences above governs this entire branch, including its `changes_security_control` pairing — by design: forcing an already-`true` value is a no-op, and this branch's actual purpose is to correct a classification the leader missed (i.e., `security_sensitive` was NOT already `true`). For a task already correctly flagged `security_sensitive: true` from Discover→classify — the ordinary majority case for a genuinely security-sensitive task, not a classification miss — this branch never fires, and so provides zero defense-in-depth against a wrongly-declared `changes_security_control: false`. That is not this branch's job: the defense for the already-known-sensitive case is `agents/architect.md § "Diff-grounded justification when declaring false on a security-sensitive task"` (SEC-DR-F2/F3), applied at Design time when `security_sensitive` is already known. As the final floor, `security`'s own Phase 3 dispatch is gated on `security_floor_applies` (`= security_sensitive == true`, § "Single shared Phase-3 floor predicate" below) alone and never depends on `changes_security_control` — so even in the worst case where a wrongly-`false` `changes_security_control` causes `adversary_floor_applies` to evaluate `false` and `adversary` to be silently skipped, `security` still independently reviews the same security-relevant change. This is why the Phase 1.5 pre-check's own fail-closed-on-ambiguity branch (above) and this backstop's fail-closed-on-ambiguity branch (immediately below) are deliberately NOT gated the same way — their trigger is inconclusiveness itself, not a confirmed miss, so pairing them unconditionally is the correct, narrower defense those two branches alone can offer.

**Fail-closed on ambiguity.** If either check is inconclusive for any reason — a path only partially matches, a command cannot run, OR the diff is unexpectedly EMPTY when changes were expected for this task (e.g., the implementer's changes are already committed/staged past the pinned base ref) — treat the task as sensitive, applying the unified rule above (also force-set `changes_security_control: true` on the same terms). An inconclusive result, including an unexpectedly empty diff, is never read as "no sensitive files, clean" and is never treated as a clean pass — and, exactly as the match branch above, it is never treated as a case where `changes_security_control` stays untouched.
...
|---|---|---|---|---|
| `1` | suite no-regress only | reduced (diff vs intent) | skipped, unless `security_sensitive: true` (then pipeline mode) | skipped, unless `adversary_floor_applies == true` (then pipeline mode) |
| `2` | default verify | validate mode | skipped, unless `security_sensitive: true` (then pipeline mode) | skipped, unless `adversary_floor_applies == true` (then pipeline mode) |
| `3` (default) | default verify | validate mode | pipeline mode | pipeline mode (if `adversary_floor_applies`) |
| `4` | default verify | validate mode | pipeline mode + extended analysis (cross-references `01-root-cause.md ## Prior Art`) | pipeline mode (if `adversary_floor_applies`) |

**Feature flow:** tester + qa always; `security` whenever `security_floor_applies == true`; `adversary` whenever `adversary_floor_applies == true` — a strict subset of `security_floor_applies` (see the shared predicates below). Each control's own predicate is its sole and complete gate; neither runs when its own predicate is false.

### Single shared Phase-3 floor predicate (T2-AC-10)

**One source of truth for `security`'s own dispatch condition.** The `security` column of the tier-gated dispatch table above, and `security`'s own dispatch clause in the Feature-flow line above, read from ONE boolean — `security_floor_applies` — computed once per task as:

```text
security_floor_applies = security_sensitive == true
```

`security_sensitive` here is the SAME field the leader set at Discover→classify AND that the Phase 2-close `security_sensitive` backstop (§ "Phase 2-close `security_sensitive` backstop", above) may force-set to `true` before Phase 3 dispatch — never a second, independently-maintained copy of the sensitivity determination. When `security_floor_applies == true`, you dispatch `security` at Phase 3: for `type: fix`/`hotfix` this is additionally gated by the tier-gated dispatch table's own `security` column (`bug_tier` can further restrict which mode it runs in); for every other type (`feature`, `refactor`, `enhancement`, or any other), `security_floor_applies` alone governs — there is no tier-table row to consult, so no further gating applies. When `security_floor_applies == false`, for `feature`/`refactor`/`enhancement`/any other non-tiered type `security` is not dispatched — the predicate is its sole and complete gate in both directions. For `type: fix`/`hotfix`, `security_floor_applies` is a FLOOR that layers on top of, but never suppresses, the tier-gated table's own independent unconditional `security` cell: at `bug_tier` 1/2 the `security` cell reads "skipped, unless `security_sensitive: true`", so `security_floor_applies == false` does mean it is skipped there; but at `bug_tier` 3/4 the `security` cell is unconditional ("pipeline mode") and fires regardless of `security_floor_applies`, because bug severity alone is an independent, pre-existing mandate. `adversary`'s own dispatch condition is a narrower, separate boolean — `adversary_floor_applies` — never `security_floor_applies` directly, at any tier; see § "Adversary floor predicate" immediately below for its full definition and every consumer site.

**Why `security_floor_applies` exists as a named predicate.** Before this predicate existed, `security`'s dispatch condition was repeated inline as the bare expression `security_sensitive == true` at each consumer site — a future edit at one site without the others could silently desync them (SEC-DR4-01, co-location desync, the same class of risk T1-AC-9 later hardened against for `adversary` via its by-name-only discipline). Naming it `security_floor_applies` closes that seam for `security`'s own dispatch condition: there is exactly one place in this file where "does the security floor apply at Phase 3" is decided for `security`, and every `security` consumer site reads it. `adversary` now derives from the narrower, separately-named `adversary_floor_applies` (below) — not a second copy of `security_floor_applies` itself.

**Preserves the "unless sensitive" guard under any lane/fast-mode skip (closes SEC-DR2-02).** `security_floor_applies` is computed from `security_sensitive` ALONE — it is never gated, ANDed, or overridden by `lane`, `fast_mode`, `[TIER: N]`, or any Simple-Mode keyword. On `lane: express` (§ "Express Lane Profile" above), `qa` is skipped and Phase 1.5/1.6's panel is carved out, but `security_floor_applies` is computed identically to `lane: full` — a sensitive-path express task dispatches `security` at Phase 3 exactly as a sensitive-path full task does, and `adversary` fires alongside it on the same terms whenever `adversary_floor_applies` is additionally true (never lane-gated — see § "Adversary floor predicate" below). No lane, trim, flag, or env-var can make either predicate evaluate differently than it would on `lane: full` for the same `security_sensitive`/`changes_security_control` values.

**The only lane that omits both lenses.** `lane: inline` never reaches you (no orchestrator is spawned for inline — see "Pipeline Flow" above), so `lane: inline` is not a value this predicate ever evaluates against. The only way both lenses are omitted anywhere in this contract is the leader's inline-only constraint-E waiver (`docs/pipeline-lanes.md § 5`), which happens entirely upstream of your own spawn and waives the two-lens floor as one atomic unit — never a single lens. Once you are spawned at all (any lane you actually run), `security_floor_applies` is the only gate, and it is never waivable from inside your own contract.

### Adversary floor predicate — `adversary_floor_applies` (R5, T1-AC-1/AC-9)

**A second, narrower predicate — never a replacement for `security_floor_applies` above.** Computed ONCE, in this same section, immediately after `security_floor_applies`:

```text
adversary_floor_applies = security_floor_applies AND changes_security_control
```

`changes_security_control` is the architect-declared Classification-block boolean (`agents/architect.md § Classification block`, mirrored into `00-state.md § Current State`). **Fail-closed default, stated explicitly:** when `changes_security_control` is absent from `00-state.md` or its value is doubtful/unresolved, it is treated as `true`. The absence of the field is NEVER interpreted as "do not dispatch the adversary" — an absent producer value fails CLOSED toward dispatch, the same discipline `security_floor_applies` already applies to an absent/unclear `security_sensitive`.

`adversary_floor_applies` is a strict subset of `security_floor_applies` by construction (an AND can never be true when its left conjunct is false): the adversary fires only on the subset of security-sensitive tasks that also change a security control, while `security` itself keeps firing on every security-sensitive task regardless of `changes_security_control` — `security_floor_applies` and its own consumer sites (the `security` column of the tier-table above, `security`'s own dispatch condition) are UNCHANGED by this predicate (Invariant B, fenced).

**Every consumer site below reads `adversary_floor_applies` BY NAME (SEC-DR-F6, T1-AC-9).** No site inlines the expansion `security_sensitive AND changes_security_control` (or `security_floor_applies AND changes_security_control`) locally — an inline expansion re-computes the gate at the consumer site and can silently drop the fail-closed default, evaluating fail-OPEN when `changes_security_control` is absent. This mirrors the producer/consumer-site desync PR #481 warned against: one named predicate, read everywhere, instead of scattered re-derivations that can fall out of sync.
...
- **adversary** (pipeline mode, when `adversary_floor_applies == true` — for `type: fix`/`hotfix` ONLY, the tier-gated table's own `adversary` column above applies ADDITIONALLY as a type-specific tier gate, since `bug_tier` can further restrict; for every other type, `adversary_floor_applies` alone governs, with no tier-table row to consult): files changed, summary, pointer to `reviews/04-security.md`, the round number `N` you have just declared in `00-state.md` (§ "Adversary per-round report-integrity scan" below), `**Re-verification scope:** full` (round 1 of a task always attacks the full surface), and `**Adversary output budget (format guidance):** ~800 + 600×(changed-control count in scope) tokens` (format guidance only — see the note below the report-integrity scan; never a cap on `break_count` or the number of controls reported, and never a budget-STOP in the sense `docs/pipeline-lanes.md § 3` prohibits). Break-the-design mandate; `broke-it | could-not-break`; `incomplete_on_changed_control: true` when a `could-not-break` verdict lands on a changed control/security-relevant path.

### Adversary per-round report-integrity scan (R2, T1-AC-5/AC-8/AC-10)

**Owner:** you — not a subagent dispatch. Runs after EVERY adversary round returns, whenever `adversary_floor_applies == true` — the initial Phase-3 dispatch above, a Case-D re-dispatch, and a staleness re-gate re-dispatch (both below) all trigger this scan. Same shape as Phase 1.5a/2.6: a deterministic Bash/file-inspection gate you run yourself, no `Task` call.

**Authoritative round counter.** You declare `adversary_rounds: N` in `00-state.md § Current State` as part of the SAME Phase Transition Protocol write that dispatches `adversary`, BEFORE the dispatch — you name `N`, then instruct the adversary to write `reviews/04-adversary-r{N}.md`. This field is what this scan and `delivery` both read as the authoritative current round; a filesystem-max glob over `reviews/04-adversary-r*.md` is explicitly the WRONG mechanism (that is exactly what check 4 below guards against).

**Migration/bootstrap rule for a task whose adversary history predates this scheme.** A task that already ran one or more adversary dispatches under the RETIRED single-accumulating-file convention (`reviews/04-adversary.md`, no `adversary_rounds`/`adversary_round_sizes`/`adversary_floor_applies` fields in `00-state.md`, no `reviews/04-adversary-r{N}.md` on disk) begins per-round-file numbering at **N=1** for its FIRST dispatch under this scheme — never a continuation of the retired scheme's own historical dispatch count. This is a one-time, explicit exception, stated here so it is never left to unwritten orchestrator judgment or silently miscounted as a round-BATCH index (AC-10's monotonic-N rule is per-DISPATCH, and a task's retired-scheme dispatches were never assigned an `N` at all — there is nothing to continue from). Checks 3 and 4 below apply ONLY to `reviews/04-adversary-r{1..N-1}.md` — files that exist under THIS scheme; a pre-existing single-file `reviews/04-adversary.md` from the retired scheme is not part of the round-file series and is never checked against either rule, and is not required to be backfilled with an `adversary_round_sizes` entry.

**Size baseline, recorded durably (closes the gap check 3 would otherwise leave open).** Immediately after check 1-4 clear for round `N` (i.e., the round is accepted), record its file's current byte size — `wc -c < reviews/04-adversary-r{N}.md` or equivalent — into `adversary_round_sizes[N]` in `00-state.md § Current State`, as part of the SAME Phase Transition Protocol write that accepts the round. This is the durable baseline check 3 reads at every LATER round's own integrity scan; without a persisted field, "the value recorded at its own round's Phase Transition Protocol write" would have nowhere to live, and a truncated prior-round report could pass check 3 by shrinking to a size no longer compared against anything real.

**N is strictly monotonic-increasing per dispatch (AC-10).** Round 1 of a task's FIRST Phase-3 adversary dispatch is `N=1`. Every subsequent adversary dispatch — a Case-D re-dispatch, a staleness re-gate re-dispatch, OR a retry of a round that FAILED this integrity scan — uses the next unused integer, `N+1`. A retry of a failed round NEVER reuses the failed round's `N`: reusing it would reintroduce the PR #494 reconstruction-overwrite defect at single-round granularity, since a second `Write` to the same `reviews/04-adversary-r{N}.md` path is exactly what the per-round file scheme (R2) exists to avoid. **This is prose-instructed discipline, not a mechanical guarantee** (consistent with how the rest of this pipeline's safety mechanisms work — no new enforcement mechanism is introduced here): the per-round-file scheme fully eliminates the FULL-FILE-RECONSTRUCTION overwrite class that caused the PR #494 incident (a single shared path silently overwritten wholesale), since each round now owns a distinct path. A narrower residual risk remains — a same-round retry that skips incrementing `N` (e.g., a session interrupted between the adversary's `Write` and the orchestrator's own completion-event write, followed by a recovery that re-declares the same `N`) — and that residual is mitigated, not structurally prevented, by the strict-monotonic-`N` discipline stated above and by § "Adversary per-round report-integrity scan" check 4 (no superior obsolete round file) at the next round's dispatch.

**Checks, run in this order:**

0. **Collision short-circuit (R2-C8) — evaluated BEFORE checks 1-4, never alongside or after them.** Read the returning agent's own status block first. If it declares `status: blocked` (the collision case — the adversary's own read-before-write step detected an existing `reviews/04-adversary-r{N}.md` and refused to overwrite it, per `agents/adversary.md § Session Context Protocol` step 5), treat this round as a FAILED round unconditionally and do NOT run checks 1-4 against it — a `status: blocked` round has no completed attack pass to check. This holds REGARDLESS of what values (if any) accompany `status: blocked` in the returning status block — even if `adversary_verdict`/`incomplete_on_changed_control`/`break_count` are present and happen to coincidentally match the pre-existing round file's own genuine verdict line, that match is never grounds to treat the round as clean. A collision return is never a benign pass.
1. **Presence + non-empty.** `reviews/04-adversary-r{N}.md` exists and is non-empty, where `N` is the round just declared in `00-state.md`.
2. **Verdict match.** The file's verdict line matches the returning agent's status-block `adversary_verdict` field (`broke-it` / `could-not-break` / `could-not-break` with `incomplete_on_changed_control: true`).
3. **No prior-round shrink or disappearance.** Every `reviews/04-adversary-r{1..N-1}.md` still exists and its size has not decreased since the value recorded in `00-state.md § Current State → adversary_round_sizes[round]` at that round's own Phase Transition Protocol write (see "Size baseline, recorded durably" above). **Absent-baseline fails CLOSED, never a vacuous pass.** If `adversary_round_sizes[round]` is itself absent for any round `1..N-1` that should have one (a `/th:recover` session that lost the map, or a round predating this field), treat check 3 as FAILED for that round — do not silently skip the comparison and report a clean pass. Re-derive the missing baseline from the file's current on-disk size before accepting the round as clean, and record it into `adversary_round_sizes` at that point so the gap does not recur at the next round. This mirrors `changes_security_control`'s own explicit fail-closed-on-absence discipline (§ "Adversary floor predicate") rather than leaving this field's absence unhandled.
4. **No superior obsolete round file (SEC-DR-F4, AC-8).** No `reviews/04-adversary-r{N+k}.md` (k ≥ 1) exists on disk. Reject any such file before accepting round `N` — it is evidence of an obsolete higher-round file left by a previous run or `/th:recover` session; check 3 (no-shrink) alone would NOT catch it, since a file that never disappears or shrinks silently passes that check while still being stale.

**Verdict handling:**

| Result | Action |
|---|---|
| `status: blocked` (collision, check 0) | Emit `adversary.report_integrity` (`verdict: fail`, `extra: {check: 0, detail: collision}`). Do NOT accept any verdict as authoritative — the round never ran an attack pass, so there is nothing to accept. Re-dispatch `adversary` with the next monotonic `N` (never the same `N` — see the monotonic-N rule above). If a second consecutive collision recurs on the retry, stop iterating and escalate to the operator — a repeated collision signals a deeper recovery/state-sync issue, not a one-off race. |
| Clean | Emit `adversary.report_integrity` (`verdict: pass`, `round: N`) to `{events_file}`. Accept the round's `adversary_verdict` into the "Gate — worst-of combined verdict" computation below. |
| Violation (any of checks 1-4) | Emit `adversary.report_integrity` (`verdict: fail`, `extra: {check, detail}`). Do NOT accept the round's verdict as authoritative. Re-dispatch `adversary` with the next monotonic `N` (never the same `N` — see the monotonic-N rule above). |
| Command/read error | Escalate — never a silent pass. `status: blocked`, surface the raw error to the operator. |

**Fails noisily, never silently accepts a shrunk report** — this is the structural close of the PR #494 incident (64k overflow + silent overwrite on retry) at per-round granularity.

**`Adversary output budget` is format guidance, never a budget-STOP (R4).** The `~800 + 600×(changed-control count in scope) tokens` figure declared in the adversary dispatch above scales the adversary's own OUTPUT with the number of controls in scope for that round; it carries no STOP, no config key, and no accumulated-cost counter, and is explicitly distinct from the `budget`-STOP pattern `docs/pipeline-lanes.md § 3` prohibits. It never caps `break_count` or the number of controls the adversary reports — compression is of FORMAT only, never of content (`docs/output-contract-patterns.md § 1`).
...
**Case-D adversary re-dispatch fields (R3, T1-AC-3).** When Case D re-dispatches `adversary` (either blast radius, and only when `adversary_floor_applies == true`), construct the payload with `**Re-verification scope:** localized {files, finding-IDs}` — the delta is the files changed since the previous round plus the specific finding under re-verification. Order the dispatch context with stable content (system prompt, CLAUDE.md, the design) at the FRONT and the delta at the END, the same prompt-caching discipline named at T2-AC-15 above. Declare the next monotonic `adversary_rounds: N` in `00-state.md` before this dispatch, per § "Adversary per-round report-integrity scan" above, and run that scan after the round returns.
...
- **R1 — owner-lens re-verification (delta-scoped).** Re-dispatch ONLY the owner lens. Owner `adversary` → carries the real `**Re-verification scope:** localized {files, finding-IDs}` field (`agents/adversary.md:118`; the Case-D contract at `:1123` above) as its delta-scope descriptor — never a `Blast radius` field substituting for it. Owner `security`/`qa`/`tester` → the delta-scope descriptor is the brief's own `Blast radius: localized {IDs}` field (these lenses have no `Re-verification scope` field of their own). Owner still open → append a brief entry and bounce to the producer (for an `adversary` owner, evaluate the whack-a-mole detector first, per `docs/patch-mode.md § Whack-a-Mole Structural Escalation`), zero non-owner-lens tokens. Owner closed → enables R2.
- **R2 — single consolidated confirmation (delta-scoped, non-owner lenses).** With every owner closed, issue exactly ONE consolidated dispatch of the non-owner lenses from this task's base-pass set, over the final patched state, delta-scoped — never a fresh full base pass. A non-owner `adversary` lens in this dispatch carries its own `**Re-verification scope:** localized {...}`, explicitly defined as the files changed since the previous round — the same delta descriptor the Case-D adversary re-dispatch fields (`:1123` above) already define, reused verbatim here rather than narrowed to the owning lens's own finding-IDs alone. The worst-of is computed over ALL lenses' final verdicts with the unchanged formula (`:1078-1085`). A fail on any lens in R2 opens a new iteration (counts against max-3).
...
**Reconciliation with the staleness re-gate below (non-negotiable, deferred by reference — not restated narrower):**
1. Owner is `security`/`adversary` and the security surface changed → the owner set clamps to BOTH `{security, adversary}` in R1 whenever `adversary_floor_applies == true` — this is the same "a stale verdict re-runs BOTH" rule stated verbatim in the staleness re-gate paragraph immediately below (not a separate or narrower rule). That pair does not then go to R2 as a delta-trust confirmation; R2 in that iteration covers `tester`+`qa` only.
2. Owner is `qa`/`tester` but the patch touched the security surface → the staleness re-gate below fires on its own FULL trigger (both limbs: a security-relevant design-surface edit, OR new implementation files touching auth/API/DB/crypto/session paths or adding/modifying any canonical-control-vocabulary element regardless of path) — never a narrower "design-surface-only" reading. Either limb makes the security+adversary portion of R2 a FRESH re-run (no skip, no delta-trust).

**Recompute-and-recache trigger extended to this branch (closes a mechanism gap the claim above used to leave open for limb (b)).** When it is specifically limb (b) that fires here — a Case-A/D patch iteration owned by `qa`/`tester` adds new implementation files touching auth/API/DB/crypto/session paths, or adds/modifies any canonical-control-vocabulary element, while `adversary_floor_applies` is still cached `false` from a base pass where `changes_security_control` was `false` — force-set `changes_security_control: true` for the remainder of the task, applying the same force-set/recompute pairing discipline § "Unified rule governing every `security_sensitive`-force-set site" above establishes for every additional force-set site added anywhere in this file (never an inline re-derivation of the pairing). This force-set fires the SAME "Recompute-and-recache `adversary_floor_applies`" mechanism (below) that already reacts to a Case-B `changes_security_control` change — it now ALSO fires on this limb-(b)-during-Case-A/D-patch trigger, so `adversary_floor_applies` is recomputed and the cached field rewritten in `00-state.md § Current State` BEFORE R2 proceeds. Without this extension, the "FRESH re-run" claim above would hold for `security` but not for `adversary` on this trigger; with it, `adversary` is dispatched on the newly-discovered control rather than silently skipped on a stale cached `false`.

**Known, disclosed limitation (this fix closes the gap only for a task with an already-recorded security/adversary verdict — it does not re-arm classification for a task never classified `security_sensitive` at all; out of scope for this fix).** The extension above operates entirely inside the Security-verdict staleness re-gate below, whose own opening sentence gates every trigger it lists — including this one — on occurring "AFTER a security/adversary verdict is recorded." That precondition holds for a task that WAS classified `security_sensitive: true` at some point (a verdict exists for the re-gate, and this extension, to apply to). It does NOT hold for a task that was never classified `security_sensitive` at Design time (correctly, at the time — the original diff touched nothing sensitive) and passed the Phase-2-close backstop cleanly, so no verdict was ever recorded. If a LATER Case-A/D patch, owned by `qa`/`tester`, introduces a brand-new security control for such a task, neither `security` nor `adversary` runs: nothing in this file re-arms `security_sensitive`/`security_floor_applies` for a task mid-Phase-3-iteration, and the Phase-2-close backstop that would have force-set that classification (§ "Coordination note — three distinct Phase-2-close mechanisms" above) runs once, before Phase 3, and is never re-triggered by a later iteration. This residual is pre-existing — independent of, and not introduced by, this cost-ordered re-run sequencing — and closing it is explicitly out of scope for this fix; closing it is recommended as a candidate for a dedicated follow-up issue tracking re-arming task security-classification mid-Phase-3-iteration, but no such issue has been filed yet and nothing in this pipeline forces one to be filed before or after this fix ships. Separately, the "recomputed... rewritten... BEFORE R2 proceeds" ordering the extension above depends on (§ "Recompute-and-recache trigger extended to this branch") is likewise **prose-instructed discipline, not a mechanical guarantee** — matching the monotonic-`N` precedent above (§ "N is strictly monotonic-increasing per dispatch"), no new enforcement mechanism is introduced here: no automated check currently confirms the recompute-and-rewrite actually completed before R2 dispatches, so a skipped or delayed recompute (e.g., a session interruption between the force-set and the orchestrator's own `00-state.md` write) could silently leave `adversary_floor_applies` stale for that dispatch with nothing to catch it.

**Reconciliation with the adversary per-round report-integrity scan.** Whichever stage (R1 as owner, or R2 as confirmation) dispatches `adversary`, declare the next monotonic `adversary_rounds: N` BEFORE the dispatch and run the per-round scan (`:1045-1074`) AFTER it returns — unchanged. Sequencing changes WHEN/HOW OFTEN these lenses run, never WHAT they do when they run.

### Whack-a-mole structural escalation — architect re-design pivot (canonical contract: `docs/patch-mode.md § Whack-a-Mole Structural Escalation`)

**Scope.** Evaluates ONLY at R1 above, and ONLY when the R1 owner set includes `adversary` — alone, or via the security+adversary clamp (§ "Reconciliation with the staleness re-gate below" item 1 above). Never at R0; at R2 only via the secondary evaluation point defined below (§ "Evaluation point (AC-25)" — the staleness re-gate forcing a FRESH security+adversary re-run inside R2). Never interacts with the structural fail-safe above: `Blast radius: structural` collapses R0 straight to the COMPLETE Case-row verifier set, bypassing R1/R2 narrowing entirely — this detector's R1-scoped evaluation point is simply not reached on that branch.

**Evaluation point (AC-25).** Immediately after an `adversary` round in R1 returns `broke-it` AND the per-round report-integrity scan above accepts the round. NEVER on a `status: blocked` or integrity-failed round — that round is re-dispatched with the next monotonic `N` first, per that scan's own verdict-handling table; invalid data is never whack-a-mole input. Secondary point: when the staleness re-gate below forces a FRESH security+adversary re-run inside R2 and that adversary round also returns `broke-it`, the same detector applies over the same round series.

**Detector — two variants, OR.**
- **New-surface (fires immediately, AC-16).** B(N) = set of control IDs with `broke-it` in `reviews/04-adversary-r{N}.md § Attempts by Control`. B(N-1) is NOT literally round N-1 — it is the break-set of the most recent round PRIOR to N that the per-round report-integrity scan (`:1045-1074`) ACCEPTED as clean, skipping any intervening integrity-rejected round (a rejected round carries no valid break-set and is never read as this baseline — invalid data is never whack-a-mole input, per the Evaluation point above). Fires when `B(N-1) \ B(N) ≠ ∅` (≥1 previously-broken control CONFIRMED closed) AND `B(N) \ B(N-1) ≠ ∅` (≥1 NEW control broken on the patch's own surface) — the moving-target signature.
- **Consecutive (threshold N=2, AC-17).** `adversary_consecutive_broke_it` (§ "Current State" schema above) increments per integrity-clean `broke-it` round on a changed/hard-point control, resets to 0 on `could-not-break`. Fires at 2 — deliberately before the implementer's max-3 budget below exhausts.

**Known, disclosed limitation (detector coverage gap — an analytical limitation of the two-variant design as approved, out of scope for a mechanism fix here).** A remediation sequence that interleaves genuine `could-not-break` rounds between recurring-but-distinct breaks defeats both variants: Variant 1 compares only adjacent rounds N-1/N, so a clean round immediately preceding a new break makes `B(N-1) \ B(N) = ∅` and the variant can never fire on that pair; Variant 2 resets its counter to 0 on any `could-not-break`, so it never reaches its threshold under the same interleaving. The pattern `broke-it, could-not-break, broke-it, could-not-break, ...` — which can arise either from a genuinely unrelated reopening of a similar flaw OR from a genuine moving-target design defect (the same interleave a real design fault produces; a missed case is not necessarily a benign one) — falls through both variants undetected regardless of cause. This is a genuine, known limitation of the two-variant design as approved at STAGE-GATE-1, not an implementation bug, and closing it (e.g., a wider comparison window across more than two rounds) is out of scope for this fix — expanding the detector's own comparison logic now would be a scope increase beyond what was approved, not a correction of what was built; it is recorded here as a candidate for a future redesign consideration, not a defect blocking this task. A case missed by both variants still falls through to the pre-existing max-3-iteration operator-escalation floor below, which DOES still catch it — only later than this detector's early-pivot optimization would have. This gap is not strictly equivalent to a pre-feature baseline, however: this feature's own delta-scoped R1 re-attack (§ "Cost-ordered re-run sequencing" above, the `**Re-verification scope:** localized {...}` field) raises the base rate of narrow, could-not-break rounds relative to a full-scope base-pass-only world — a narrow-delta `could-not-break` is weaker evidence than the full-scope one it replaces, yet Variant 2 zeroes its counter identically on either — so the interleaving-evasion pattern is relatively MORE reachable under this feature's own cost-ordered re-run sequencing than it would be in a hypothetical world without it. This is a known gap in the added optimization's coverage, not a new regression or a security bypass.

**Action on trigger (AC-18–AC-22, AC-28).** 1) Does NOT consume an implementer iteration — the triggering round is the terminal signal, not a fresh bounce. 2) Preempts R2 for this iteration — R2's precondition ("every R1 owner closed") was never met once `adversary` returned `broke-it`, so R2 was never going to run; the pivot below replaces the default bounce-to-producer. 3) **IF `structural_escalations` is below its cap (default 1):** pivots to a FRESH `architect` re-design (Case B structural form) carrying the ACCUMULATED break-set — every break vector from every round belonging to the CURRENT whack-a-mole detection episode for the CURRENT task, spanning the persisted `whack_a_mole_episode_start_round` field (§ "Current State" schema above) through the triggering round — a NAMED, persisted field, never unstructured `00-state.md` history reconstruction — never only the triggering round or the original AC alone, and never a filesystem-wide glob over `reviews/04-adversary-r*.md` — this pipeline's `adversary_rounds` counter is shared and monotonic ACROSS TASKS in a multi-task run, so an unscoped glob would pull an unrelated, already-closed task's break vectors into this task's redesign spec — as the design specification. Construct the re-design payload to explicitly INSTRUCT external research before designing: WebSearch/WebFetch for known solution patterns/mitigations for the broken control's problem class, plus context7 when a third-party API/library is involved (a producer/consumer directive — passive tool availability alone is not sufficient). Require the resulting design to carry a prior-art/research-citation section, citing findings or explicitly stating none applicable were found. **ELSE (the cap is already consumed):** escalates directly to the operator (`status: blocked`) with the accumulated break-set and all prior design iterations, per the Budget interaction paragraph below — never a second redesign. 4) Re-runs plan-review (Phase 1.6) + security design-review (Phase 1.5 panel) before re-implementing — never fast-tracked to the implementer. 5) Consumes one structural escalation; re-implementation runs a FRESH Phase-3 base pass (all base-set lenses, full new diff) — never a continuation of the pre-escalation R0/R1/R2 iteration; verdicts tied to the old design model are void.

**Budget interaction (AC-23, AC-24).** `structural_escalations` (§ "Current State" schema above) caps redesign pivots at 1 by default; a SECOND whack-a-mole after the redesign escalates to the operator (`status: blocked`) with the accumulated break-set and both design iterations — never an unbounded redesign loop. The implementer's max-3 budget above is reserved for genuine implementation bugs against the CURRENT design; the fresh base pass after a **detector-triggered** redesign gets a FRESH max-3 implementer budget, since it targets a new design model. A `structural` redesign reached via the Case → routing table (Case B, `structural` blast radius) rather than via this detector does NOT reset the implementer budget through this mechanism — that path is governed entirely by the pre-existing Case-routing rules above and is unrelated to `structural_escalations`, which increments ONLY on a detector-triggered redesign. **`structural_escalations` is MONOTONIC per task and is NEVER reset by a redesign or a fresh base pass — only the implementer iteration budget resets** (corrected per `reviews/01-plan-review.md § Security Design-Review`): resetting the escalation counter itself would let the cap=1 floor be defeated by an unbounded redesign→fresh-pass→redesign sequence, each individually "fresh" but collectively unbounded. **The fresh base pass following a redesign zeroes `adversary_consecutive_broke_it`** — the counter is bound to the discarded design model's controls; carrying it forward against a design the adversary has not yet attacked would let a stale count trigger the consecutive variant prematurely.

**Prose-instructed discipline, not a mechanical guarantee (matching the monotonic-`N` precedent § "N is strictly monotonic-increasing per dispatch" above; no new enforcement mechanism is introduced here).** Three residuals in this section share that character, none independently gated by an automated check: (1) the cap-and-escalate sequence above has no wired gate between "a second trigger is detected" and "`structural_escalations` is persisted and the operator is notified" — a session interruption between those two steps could silently lose the increment or the escalation, letting a redesign proceed as if it were the first; (2) the external-research directive (step 3, AC-28, above) is a dispatch-construction instruction with no downstream check confirming it was included in the payload or that the resulting design's prior-art section reflects genuine research rather than a placeholder; (3) the detector's own evaluation point (above) has no persisted `00-state.md` marker recording "evaluated for round N," so a session interruption between an integrity-clean `broke-it` round and the detector's evaluation could silently skip that round's check.

**Fail-closed-on-unconfirmable-persistence-state (AC-30) — closes residual (1)'s compounding risk.** Whenever the orchestrator cannot positively confirm, from `00-state.md` and its corroborating trace (`{events_file}`), that the persisted `structural_escalations`/`whack_a_mole_episode_start_round` values accurately reflect every redesign this task has actually undergone — e.g., on resume after an interruption, when evidence of a redesign exists (a landed architect revision to `01-plan.md`, a fresh base pass having run) without a matching persisted escalation increment — the orchestrator FAILS CLOSED: treats the cap as already consumed (never as available), escalates directly to the operator (`status: blocked`) with the ambiguity stated explicitly, and NEVER grants a FRESH implementer budget against unconfirmed persistence state. This mirrors the existing fail-closed-on-absence discipline already applied to `changes_security_control` (§ "Adversary floor predicate") and to the `adversary_round_sizes` absent-baseline check (§ "Adversary per-round report-integrity scan" check 3) — unlike those two fields, there is no sound way to re-derive a lost escalation COUNT from on-disk evidence alone, so escalating to the operator (not a safe-substitute recompute) is the only sound resolution. This converts residual (1)'s previously-unbounded redesign-compounding risk into a bounded operator escalation, while leaving the fresh-max-3-budget-per-redesign policy (AC-24, § "Budget interaction" above) intact for the ordinary, confirmed-state case.

Residuals (2) and (3) remain bounded exactly as before — fail-SAFE (a missed trigger costs a delayed escalation or an extra tactical iteration, never a silent security bypass), backstopped by the pre-existing max-3-iteration operator-escalation floor below. Residual (1)'s own ultimate bound is now the fail-closed-escalate rule immediately above, not max-3.

**No parallel dispatch predicate (AC-26).** Reads existing state only — the adversary round-report series, `adversary_verdict`, the authoritative `adversary_rounds: N` counter, and the per-control break-sets derived from those reports — and adds exactly two new `00-state.md` fields (`adversary_consecutive_broke_it`, `structural_escalations`). Introduces no rival dispatch predicate; `adversary_floor_applies`/`security_floor_applies`/the Case → routing table/the worst-of formula stay verbatim. A third field, `whack_a_mole_episode_start_round` (§ "Current State" schema above), is added purely as episode-boundary bookkeeping — it is read-only, consumed by § "Action on trigger" step 3's round-range computation, never a dispatch predicate, so it does not affect this paragraph's no-rival-predicate claim.

**Known, disclosed limitation (the setter fix above closes the limb-(b) path named at round 8 — it does not cover every mechanism that can first-dispatch `adversary` for a task; out of scope for this fix).** The setter's branch (b) is scoped specifically to the "Recompute-and-recache trigger extended to this branch" mechanism (§ above) — a Case-A/D patch, owned by `qa`/`tester`, that force-sets `changes_security_control: true` mid-iteration. It does NOT cover a distinct, pre-existing mechanism: a task already classified `security_sensitive: true` at Design (so a `security` verdict is already on record, satisfying the staleness re-gate's own "AFTER a security/adversary verdict is recorded" precondition) but `changes_security_control: false` at the base pass (so `adversary_floor_applies` was cached `false` and `adversary` never ran at the base pass) can later have `changes_security_control` flipped to `true` by a Case-B architect correction — firing the staleness re-gate's own 4th trigger bullet (§ "Security-verdict staleness re-gate" below) and the OLDER "Recompute-and-recache `adversary_floor_applies` whenever this re-gate fires on a `changes_security_control` change" mechanism (below), a genuinely different code path from the extension above. On this route, `adversary` is also dispatched for the first time for that task, and neither of this field's two setter branches fires — `whack_a_mole_episode_start_round` stays `null`, reproducing the same undefined-round-range worst case (§ "Action on trigger" step 3) via a route this fix does not touch. This is bounded, not a security bypass: `security`/`adversary` themselves still run correctly on this route; the gap affects only the completeness of the accumulated break-set fed to a hypothetical future architect redesign, on a narrow precondition (a task already-classified-sensitive later gaining a security-relevant control via a plan correction, not via ordinary implementation). Closing it requires generalizing the setter to a mechanism-agnostic rule (set the field on ANY first dispatch of `adversary` for the task's current episode, regardless of which named mechanism caused it) — recommended as a candidate for the same follow-up issue tracking re-arming task security-classification mid-Phase-3-iteration (§ above), since both residuals share the same root shape: a setter/classification mechanism enumerated by NAME rather than by a general trigger-agnostic condition.

**Byte-consistency requirement (2-site invariant, AC-27).**

| Site | File | Anchor |
|------|------|--------|
| Canonical contract | `docs/patch-mode.md` | § Whack-a-Mole Structural Escalation |
| Orchestrator wiring | `agents/orchestrator.md` (this section) | § "If any agent fails → ITERATE" — whack-a-mole escalation subsection |

A future edit to the escalation rule at one site without the other desynchronizes the contract — the same failure mode the cost-ordered re-run sequencing's own 3-site invariant above already guards against.

**Security-verdict staleness re-gate (applies regardless of blast radius or case type).** A security/adversary verdict is BOUND to the security-relevant design surface it reviewed at issue time. That surface includes: the enforcement model, status codes that gate access, rollout order of controls, AND-gate conjuncts, kill-switches, feature flags, observe-window presence, and — named explicitly, not left as an inferred case of "the enforcement model" — a validation, an allowlist, a rate limit, an early-return, or an error handler (the same canonical control vocabulary `agents/architect.md § Classification block` and `agents/adversary.md § Method` both cite; a Case-A/Case-D patch adding one of these outside an auth/API/DB/crypto/session path is a security-relevant surface change even though it may not match the narrower path-based trigger below). When any of the following occurs AFTER a security/adversary verdict is recorded, the verdict is STALE and the security stage — `security` unconditionally on the security-relevant surface, `adversary` additionally when `adversary_floor_applies == true` — MUST re-run before delivery or push proceeds: an operator "simplify/remove" edit modifies or removes any element of the security-relevant design surface (even if the edit seems benign — fail-SAFE on doubt); new implementation files are committed that touch auth/API/DB/crypto/session paths not part of the reviewed surface **OR that add/modify any element of the canonical control vocabulary above, regardless of which file or path it lands in** — a validation/allowlist/rate-limit control added to a file outside the auth/API/DB/crypto/session path list is still a security-relevant surface change and still fires this re-gate; a diff-hash/mtime check shows the security-relevant design surface changed since the last recorded verdict; **a Case-B (`localized` or `structural`) architect correction changes the `changes_security_control` value** — this is itself an instance of "the security-relevant design surface changed since the last recorded verdict" (the same bullet immediately above), named explicitly here so the trigger is not left as an inferred case. The `adversary` re-dispatch this re-gate triggers carries `**Re-verification scope:** localized {the changed surface}` — never `full`, since the re-gate is itself a delta-scoped re-verification of what changed since the last verdict — and uses the next monotonic `adversary_rounds: N` per § "Adversary per-round report-integrity scan" above. Order the dispatch context with stable content (system prompt, CLAUDE.md, the design) at the FRONT and the changed surface at the END, the same prompt-caching discipline named at T2-AC-15 above and applied identically to the Case-D re-dispatch fields above.

**Recompute-and-recache `adversary_floor_applies` whenever this re-gate fires on a `changes_security_control` change.** Because `delivery` (`agents/delivery.md`) trusts the cached `adversary_floor_applies` value in `00-state.md § Current State` verbatim and never re-derives it locally (SEC-DR-F6), a `changes_security_control` flip that fires this re-gate is not fully handled by re-running `security`/`adversary` alone — you MUST also recompute `adversary_floor_applies = security_floor_applies AND changes_security_control` with the corrected `changes_security_control` value and rewrite the cached field in `00-state.md § Current State` BEFORE the re-run's verdict is recorded, so that any later read (Phase 3.5's Acceptance Gate, or `delivery`'s own existence-gate check) sees the corrected value rather than the stale one computed before the correction. This keeps the recompute entirely on the orchestrator side of the producer/consumer boundary — `delivery` still never locally re-derives the predicate.
