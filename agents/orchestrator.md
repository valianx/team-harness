...
**Scope.** Evaluates ONLY at R1 above, and ONLY when the R1 owner set includes `adversary` — alone, or via the security+adversary clamp (§ "Reconciliation with the staleness re-gate below" item 1 above). NEVER evaluates at R0 or R2. Never interacts with the structural fail-safe above: `Blast radius: structural` collapses R0 straight to the COMPLETE Case-row verifier set, bypassing R1/R2 narrowing entirely — this detector's R1-scoped evaluation point is simply not reached on that branch.
