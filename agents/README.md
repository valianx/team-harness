...
|---|---|---|---|---|
| `orchestrator` | opus | `high` | Read, Edit, Write, Bash, Glob, Grep, Task, WebFetch, WebSearch, NotebookEdit, all 9 `mcp__{context-harness,memory}__*` (KG read + write, dual backend) | Central hub. Coordinates the pipeline and routes to all other agents. |
| `architect` | opus | `max` | Read, Glob, Grep, Edit, Write, WebFetch, WebSearch, `mcp__{context-harness,memory}__*` (search + open, dual backend) | Architecture design, research, planning, audits. **No Bash** (read-only on system). KG read-only (Phase 6 writes stay in orchestrator). |
| `agent-builder` | opus | `max` | Read, Edit, Write, Glob, Grep, Bash | Create / improve agents and skills. |
| `security` | opus | `max` | Read, Glob, Grep, Edit, Write, WebFetch, WebSearch, `mcp__{context-harness,memory}__*` (search + open, dual backend) | OWASP / CWE / ASVS audits. **No Bash** (strict read-only on system). KG read-only for prior-vuln lookup. |
| `reviewer` | opus | `max` | Read, Glob, Grep, Edit, Write, Bash | GitHub PR review. Bash limited to `git`/`gh` for diff retrieval. |
| `qa` | opus | `high` | Read, Glob, Grep, Edit, Write, `mcp__{context-harness,memory}__*` (search + open, dual backend) | Acceptance criteria definition and validation. **No Bash** (read-only on system). KG read-only for AC-pattern lookup. |
...
| `tester` | sonnet | `medium` | Read, Edit, Write, Bash, Glob, Grep, `mcp__{context-harness,memory}__*` (search + open, dual backend) | Test suites with factory mocks. KG read-only for test-pattern lookup. |
