...
One agent owns coordination. **`orchestrator`** is the top-level session agent and the operator's single point of contact — it owns Intake, Discover/framing, Specify, spec+AC co-authoring, config/language resolution, initiative/`overview.md` ownership, and the gated execution pipeline. It presents every STAGE-GATE to the operator inline and records the release itself — there is no second agent to relay through. It dispatches specialists directly for both gated pipeline phases and the lighter non-gated direct modes (research, docs, mentor, init, translator, define-ac), and never dispatches another coordinator, including another copy of itself. The canonical runtime hierarchy — who dispatches whom, and where each gate lives — is [`docs/agent-tree.md`](../docs/agent-tree.md).

| Agent | Objective | Model | Effort | Tools (allowlist) | Role |
|---|---|---|---|---|---|
| `orchestrator` | Run intake, spec co-authoring, the gated execution pipeline, gate presentation, and delivery; dispatch specialists, never another coordinator. | opus | `high` | Read, Edit, Write, Bash, Glob, Grep, Task, WebFetch, WebSearch, NotebookEdit, `mcp__memory__search_nodes`, `mcp__memory__open_nodes`, `mcp__memory__create_nodes`, `mcp__memory__add_observations`, `mcp__memory__create_relations`, `mcp__memory__read_graph`, `mcp__memory__session_start`, `mcp__memory__session_end`, `mcp__memory__record_flow_event` | Top-level session agent + operator's single point of contact. Owns Intake, Discover/framing, Specify, spec+AC co-authoring, config/language, initiative/`overview.md`. Runs Phase 1 Design → Phase 6 Knowledge Save, dispatching the leaf specialists directly, and **prepares, presents and records all three STAGE-GATEs** itself — sole writer of its own `00-state.md`. On CC it runs as the session agent (never dispatched as a subagent), so its effective model and effort are the session's, not this frontmatter; on `opencode` the `primary` tier is granted by the installer's role-override layer. Opens the KG session at Intake (`session_start`) and runs its own up-front `search_nodes` query; all other KG operations (reads, writes, and `session_end`) are its own as well. |
...
- `_shared/output-template.md` — cross-cutting output-discipline contract (silence-on-success, one-line error + next-step). Consumed by `orchestrator.md`, `delivery.md`, `init.md`, `architect.md`, `implementer.md`, `tester.md`, `qa.md`, `security.md`, and the setup/lint/memory skills.
...
**Tally (standard mode):** core `opus` agents — `orchestrator`, architect, agent-builder, security, qa-plan, mentor (plus the GCP/UX specialists); `haiku` — `researcher`, `init`; everything else on `sonnet`, including `adversary`, `reviewer`, `reviewer-consolidator`, `translator`. In low-cost mode, all on `sonnet`. No `max`, no `low`.
