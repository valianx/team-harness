...
| `reviewer-consolidator` | Merge selected reviewer/QA/security drafts into one concise review; never edits source files. | sonnet | `medium` | Read, Edit, Write, Glob, Grep | De-duplicates logical findings, adjudicates evidence, and separates body-only cross-file findings from inline threads. |
