...
| `qa-plan` | Define sound acceptance criteria and semantically ratify plans before code; never writes code. | sonnet | `high` | Read, Glob, Grep, Edit, Write | Pre-code AC work: define-ac and ratify-plan. Mechanical plan checks stay in Phase 1.5a; post-code constraint choices stay with the operator. **No Bash**. |
| `qa` | Validate an implementation against its acceptance criteria and report pass/fail; never writes code or defines AC standalone (that lives in `qa-plan`). | sonnet | `high` | Read, Glob, Grep, Edit, Write, `mcp__memory__search_nodes`, `mcp__memory__open_nodes` | Post-code validation: validate (Phase 3), docs-validation, cross-repo review. **No Bash** (read-only on system). KG read-only for AC-pattern lookup. |
