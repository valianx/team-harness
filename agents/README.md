...
- `model` — `opus` for the highest-stakes, non-recoverable analysis and coordination (`architect`, `security`, `agent-builder`, `lider`); `sonnet` for everything else — execution against a plan AND secondary analysis/review a human or gate still ratifies (`adversary`, `reviewer`, `qa`, `acceptance-checker`, `translator`, …), since Sonnet 5 at `xhigh` sits near Opus for these at a fraction of the cost; `haiku` only for `researcher`/`init` (mechanical, downstream-gated).
...
Coordination is split across two agents. **`lider`** is the top-level session agent and the operator's single point of contact — it owns Intake, Discover/framing, Specify, spec+AC co-authoring, config/language resolution, and initiative/`overview.md` ownership. At each gate it presents the STAGE-GATE to the operator inline and relays the operator's decision (verbatim, tagged `lider-relayed-operator`) to the orquestador, which records it — it never writes a gate-release itself and owns no pipeline `00-state.md`. It spawns one **`orquestador`** per task (or per project in a multi-project initiative) to run Phase 1 Design → Phase 6 Knowledge Save, and that instance prepares and records all three STAGE-GATEs and is the sole writer of its own `00-state.md`. `lider` also dispatches non-gated specialists directly for the lighter direct modes (research, docs, mentor, init, translator, define-ac). The canonical runtime hierarchy — who dispatches whom, and where each gate lives — is [`docs/agent-tree.md`](../docs/agent-tree.md).

| Agent | Model | Effort | Tools (allowlist) | Role |
|---|---|---|---|---|
| `lider` | opus | `xhigh` | Read, Edit, Write, Bash, Glob, Grep, Task, WebFetch, WebSearch, NotebookEdit, KG read + `mcp__memory__record_flow_event`/`session_*` | Top-level session agent + operator's single point of contact. Owns Intake, Discover/framing, Specify, spec+AC co-authoring, config/language, initiative/`overview.md`. Spawns one `orquestador` per task; dispatches direct-mode specialists itself. **Presents + relays** — presents each STAGE-GATE to the operator inline and relays the decision (tagged `lider-relayed-operator`); never records a gate-release and owns no pipeline `00-state.md`. On CC it runs as the session agent (never dispatched as a subagent), so its effective model is the session model, not this frontmatter. |
| `orquestador` | sonnet | `xhigh` | Read, Edit, Write, Bash, Glob, Grep, Task, WebFetch, WebSearch, NotebookEdit, KG read + `mcp__memory__record_flow_event`/`session_*` | Task-scoped execution engine — one per task/project, spawned by `lider`. Runs Phase 1 Design → Phase 6 Knowledge Save, routes to the leaf specialists, **prepares and records all three STAGE-GATEs** (`lider` presents each inline and relays the decision back), and is the sole writer of its own `00-state.md`. Never spawns another `orquestador` or a `lider`. |
| `architect` | opus | `xhigh` | Read, Glob, Grep, Edit, Write, WebFetch, WebSearch, `mcp__memory__search_nodes`, `mcp__memory__open_nodes` | Architecture design, research, planning, audits. **No Bash** (read-only on system). KG read-only (Phase 6 writes stay in `orquestador`). |
...
| `reviewer` | sonnet | `xhigh` | Read, Glob, Grep, Edit, Write, Bash | GitHub PR review. Bash limited to `git`/`gh` for diff retrieval. |
...
| `acceptance-checker` | sonnet | `high` | Read, Glob, Grep, Write | External audit comparing original spec vs delivered artifacts (Phase 3.6, non-binding verdict). **No Bash, no Edit** (write-only on its own workspace doc). |
...
| `reviewer-consolidator` | opus | `high` | Read, Edit, Write, Glob, Grep | Merges 2-3 focused review drafts (security/architecture/style) into a single unified review. De-duplicates findings, surfaces contradictions, determines verdict. Invoked by `orquestador` after parallel focused reviewer passes in multi-reviewer mode. |
...
| `researcher` | haiku | `medium` | Read, Glob, Grep, WebFetch, WebSearch, Write | Parallel web research map agent. Receives one narrow search angle, runs WebSearch + WebFetch, returns structured evidence-only findings (`claim` + `source_url` + `verbatim_excerpt` + `confidence`). Never concludes, never recommends — evidence collection only. Dispatched by `lider` (research direct mode) as N parallel lanes (default 3, cap 5). |
| `code-researcher` | sonnet | `high` | Read, Glob, Grep, Bash, Write | Parallel codebase research map agent (read-only). Receives one narrow code angle (a subsystem path-set, concern, or question facet), investigates real files via Read/Glob/Grep + read-only git introspection, and returns file:line-grounded evidence. Never concludes, never ranks, never recommends. Dispatched by `lider` (research-code direct mode) as N parallel code lanes. |
...
Plus reference files (`ref-direct-modes.md`, `ref-special-flows.md`) loaded on-demand by the coordination agents (`lider` and `orquestador`). They are not invocable subagents — their `model` field is vestigial and not enforced by `/th:lint`.

Plus two cross-cutting snippets in `_shared/` (not invocable agents), installed to `~/.claude/agents/_shared/`:

- `_shared/gh-fallback.md` — single source-of-truth fallback patterns for graceful degradation when the `gh` CLI is unavailable. Consumed by `delivery.md`, `lider.md`, `ref-special-flows.md`, and skills `issue.md`, `plan.md`, `design.md`, `define-ac.md`, `audit.md`, `review-pr.md` via cross-references.
...
`acceptance-checker` and `translator` were promoted `haiku → sonnet`: acceptance-checker is a drift-detecting gate whose miss weakens delivery review, and translator is a rare-but-must-not-fail role (neutral-register translation) — both earn a stronger model even though their downstream safety nets (non-binding verdict; human i18n-diff review) remain.
...
**Low-cost matrix** (vestigial — Go installer infra decommissioned 2026-06-02; `cmd/install/modes.go::lowCostMatrix` is no longer the source of truth. Table kept for historical reference only. Its **Standard** columns predate the `adversary`/`reviewer` → `sonnet` and `acceptance-checker`/`translator` → `sonnet` re-tiering and the `max` → `xhigh` ceiling — the Roster table above is canonical.):

| Agent | Standard model | Standard effort | Low-cost model | Low-cost effort | Notes |
|---|---|---|---|---|---|
| `lider` | opus | xhigh | sonnet | high | Top-level coordination (Intake/Specify/spec+AC/config); presents + relays gates, spawns one `orquestador` per task. Reflects the post-split roster; the legacy Go matrix predates it. |
| `orquestador` | sonnet | xhigh | sonnet | high | Task-scoped execution engine; prepares and records all three STAGE-GATEs, so effort stays high in low-cost so gate logic executes correctly. Reflects the post-split roster; the legacy Go matrix predates it. |
...
**Tally (standard mode):** core `opus` agents — `lider`, architect, agent-builder, security, reviewer-consolidator, qa-plan, mentor (plus the GCP/UX specialists); `haiku` — `researcher`, `init`; everything else on `sonnet`, now including `orquestador`, `adversary`, `reviewer`, `acceptance-checker`, `translator`. In low-cost mode, all on `sonnet`. No `max`, no `low`.
