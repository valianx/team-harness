...
When you select `low-cost` during install (or set `INSTALL_MODE=low-cost`), the installer rewrites the `model:` and `effort:` frontmatter of every agent file **in-flight** during the copy into `~/.claude/agents/`. The source files in `agents/*.md` are never modified. To switch back, re-run the installer with `INSTALL_MODE=standard` (or accept the default at the interactive prompt).
