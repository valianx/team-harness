...
| `qa` | Validate an implementation against its acceptance criteria and report pass/fail; never writes code or defines AC standalone (that lives in `qa-plan`). | sonnet | `high` | Read, Glob, Grep, Edit, Write, `mcp__memory__search_nodes`, `mcp__memory__open_nodes` | Post-code validation checkpoint, docs-validation, cross-repo review. **No Bash** (read-only on system). KG read-only for AC-pattern lookup. |
...
- `_shared/output-template.md` — cross-cutting output-discipline contract (silence-on-success, one-line error + next-step). Consumed by `orchestrator.md`, `delivery.md`, `init-project.md`, `architect.md`, `implementer.md`, `tester.md`, `qa.md`, `security.md`, and the setup/lint/memory skills.
...
1. **Model by nature of the work.** The highest-stakes, non-recoverable analysis and coordination (architect, security, agent-builder, `orchestrator`, and the GCP/UX specialists) runs on `opus`. Everything else runs on `sonnet`, including explicitly requested plan review. Only `researcher` and `init-project` stay on `haiku`.
