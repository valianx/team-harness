...
One agent owns coordination. **`orchestrator`** is the top-level session agent and the operator's single point of contact. Its startup kernel serves direct work by default; `/th:pipeline` explicitly activates the lazy-loaded gated contract in `ref-pipeline.md`. During an active pipeline it owns Intake, Discover/framing, Specify, spec+AC co-authoring, config/language resolution, initiative state, and every STAGE-GATE. It never dispatches another coordinator.
...
| `adversary` | Try to break every changed security control in the frozen delivery diff and return `broke-it` or `could-not-break`; never certifies. | sonnet | `xhigh` | Read, Glob, Grep, Edit, Write | Independent Phase-3 adversarial reviewer, dispatched only when the security floor applies. **No Bash or external search.** Writes only its own English report; `Edit` supports recovery and amend reports without touching source. |
...
| `implementer` | Write the smallest approved production diff; may update one explicitly planned canonical document when public behavior would otherwise leave it false. Never designs architecture or writes tests. | sonnet | `high` | Read, Edit, Write, Bash, Glob, Grep, NotebookEdit | Production code and narrowly required canonical documentation following the architect's Work Plan. |
...
| `delivery` | Turn reviewed evidence into the changelog fragment, acceptance matrix, and PR-body draft; never modifies tracked product/docs surfaces or performs git/GitHub mechanics. | sonnet | `medium` | Read, Edit, Write | Post-gate publication prose only. |
...
**Counting test — per lens, not per step.** An agent earns one objective per lens (concern), never one per procedure step it happens to execute. `delivery` is a worked example: changelog classification, acceptance-matrix assembly, and PR-body drafting are three steps under one publication-prose lens. Counting those steps as separate objectives would misstate the role.
