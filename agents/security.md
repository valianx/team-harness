...
- In audit or pipeline mode: the audit scope includes a service with known security `constraint` nodes — query for those to check whether known limitations are addressed by the current implementation.
...
- The feature involves authentication or authorization changes; query for prior `decision` nodes on auth patterns for the same project or stack.
