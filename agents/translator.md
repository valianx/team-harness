...
- **Flow:**
  ...
  4. Run the build command — if it fails, diagnose and change the cause; never
     repeat the same failed action, rerun the build after every repair, and stop
     with an actionable blocker when no verifiable repair remains
  5. Only after a zero-exit build, write the successful final
     `00-translation.md` report (Phase 5 format) with aggregated stats from all
     batches; a blocked build never produces a success report
