...
|---|---|---|
| `## Review Summary`, `## Architecture` (Work Plan, Services Touched, assessments), `## Task List` (task sections, AC text, Files, Delivery Grouping) | architect | author; on amend, reconcile-in-place (overwrite superseded canonical fields so each appears exactly once) |
...
| `Status:` field on task headers | orchestrator / delivery | field edit in place |
