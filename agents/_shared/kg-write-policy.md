...
**Origin of the `session_id`.** `th:orchestrator` calls `mcp__memory__session_start` at
Intake (step 2) and writes the returned UUID to
...
by `mcp__memory__session_end` at Phase 6, in the same agent.
