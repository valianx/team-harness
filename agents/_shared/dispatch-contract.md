...
would otherwise back these rules are unwired as of v2.139.0 (`CLAUDE.md § "5. Architectural Conventions"`), so `prose-only` is the honest state for most rows here, not
an evasion of the rubric's intent.

| Control | Enforcer | Failure direction | Invoker | Read at |
|---|---|---|---|---|
| Canonical rule lives in exactly one file (this one) | review, not a check | fail-closed by convention — a duplicate in another `ref-*.md`/`_shared/*.md` file is a defect, not "still present, therefore fine" | reviewer reads both sites | not mechanically enforced |
