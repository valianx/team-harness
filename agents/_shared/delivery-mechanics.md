...
reported by the non-mutating remote-tip check before push and by the later PR merge-state poll;
...
## 5. Report merge state once

When a PR number is known, poll mergeability at 0s, 2s, and 4s only while
GitHub reports `UNKNOWN`:
...
snapshot. Do not wait for CI. `BEHIND`, `DIRTY`, or another base condition is a
...
Then write terminal artifacts/events and set `phase/status: complete`.
...
| Merge-state/CI poll | report-only; never wait or mutate |
