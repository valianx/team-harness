...
- Validation in the guided lane confirms rather than iterates: one full-scope review, after which every blocking finding is classified against the bound criteria. A covered finding closes by executing its criterion's scenario and the deterministic suites; an uncovered finding above the floor returns to the authored change for an operator-approved revision; a sub-floor finding rides as a pull-request concern. None opens another review, and a reviewed closure pass over a fix runs only on an explicit live operator request.
...
- The review surface may exclude the exact changed paths a locally executed deterministic checker proves byte-identical to their canonical source, under preconditions that keep the exclusion auditable and keep the independently derived scan list consistent. A derived artifact, a hand-authored generator input, and any path no checker proves stay in review by construction.
