...
- Refresh writes a replaceable latest-context/latest-conversation pair and uses a separate latest snapshot, preserving the original context, Git objects, frozen worktree, reports and drafts. Fetch/repack in the original bare repository was rejected because a force-push could make previously captured base objects unreachable. Existing safe pair promotion preserves the last observation on a failed refresh. Replacing the original context was rejected because it blurs which code and intent were reviewed.
