...
- Validate consolidation, adaptive readiness and supported runtime transformations with frozen fixtures and focused regressions. Keep the broader token/latency comparison as a deferred #662 experiment; this delivery makes no measured-savings claim.
...
Canonical review skills and role contracts, the existing review helper, Codex projections, Claude plugin assets, OpenCode adapters/distribution and targeted tests. Preserve public invocations, policy options, immutable snapshots, independent blocker verification and publication approval. Completion is bundled with `workflow-only-cleanup` in the next PR as requested on 2026-09-19.
