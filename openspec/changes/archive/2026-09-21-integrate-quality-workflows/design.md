...
Diagnostic audit/find-bugs uses an existing project-managed CLI installation first. Ordinary review-pr uses an official host/tool-cache executable outside the reviewed project and snapshot, never a project-provided binary, dependency or executable configuration. A tool-only installation uses the official package manager outside tracked product files; deliberately adding a maintained development dependency or project analysis configuration is a separate concrete product change within scope. Do not alter dependency manifests merely to hide task-local setup. Preparation may use official package managers, while analysis uses resolved executables. Existing hermetic quality-runner restrictions remain intact; this direct diagnostic integration does not require routing scans through that runner.

Resolve TH versus Sentry ownership before setup/update. TH's new whole-project capability projects to `th-find-bugs` in OpenCode's flat skill namespace; Sentry retains `find-bugs`. Claude Code and Codex retain their namespaced TH plugin entries. OpenCode adapters resolve canonical shared-agent documents under the existing `th-references/agents` installation, keeping reference documents separate from invocable agents.
