...
- `codex-runtime-parity`: portable review prerequisites and reviewer readiness.

Archive reconciliation preserves reload's activation reporting while adopting
`workflow-guidance-followup` for review admission: unavailable loaded-byte
attestation alone does not block a verified native read-only reviewer.
