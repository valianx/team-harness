...
The review summary SHALL report known criterion matches and unknown coverage honestly. Missing criterion mapping MUST NOT be interpreted as an authored-spec defect. Main determines whether a finding requires a code correction, an intent change or a documented concern.
...
- **THEN** the summary reports the known criterion match and Main verifies the affected property when closing the finding.
