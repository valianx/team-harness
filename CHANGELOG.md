...
### Changed
- Design delivers one solution proposal for product and development, with functional content first and technical detail afterward.
...
### Fixed
- Remove obsolete specialist contracts and report prerequisites; align test ownership and delivery with shared skills.
...
- Inline review reports factual evidence through `summary`; `gate` remains an alias and no longer computes publication readiness.

### Fixed
- Preserve reviewer locations expressed as `path`/`line` or structured evidence without inventing criterion coverage.
- Avoid false executable-mode drift on native Windows and support long paths in isolated PR snapshots without changing global Git settings.
...
- Retire the developer-mode replacement style and forced startup identity while retaining native-agent workflow discovery, specialist coordination, voice, language and workspace/Obsidian support.
- Setup and update stop copying the style and guide migration of existing selections while preserving customized files and unrelated settings. Execution guards remain unchanged.
...
- OpenSpec publishes PRs after complete reviews and verified blocker closure without requiring another reviewer pass.
...
- Added `CLAUDE.md` as an additive Codex project-instruction fallback during
  generated project setup and global setup/update reconciliation, while
  preserving `AGENTS.md` precedence and existing operator fallbacks.
...
- Added a native read-only inline reviewer for high-quality review outside a pipeline workspace,
  while preserving the full immutable-snapshot `review-pr` workflow for pull requests.
...
- Unified the Claude Code plugin, Codex plugin, installer, and release tag under version 3.6.0; the tagged Git tree is the Codex artifact rather than a separate archive.
...
- Bound PR reviews to immutable head, base, merge-base, commit, and conversation snapshots.
- Preserved paginated review threads and prior review history while preventing stale draft publication.
...
### Changed
- **Single owner for coordination state.** `architect` decides the nine classification values and
  returns them structurally; the coordinator validates and transcribes. No specialist writes
  `00-state.md`. The same split resolves `reviews/01-plan-review.md`: the panel owns it, and the
  architect's closure rubric moves to its own file.
- **The express gate is named for what it is** — a review-and-ship gate, prepared after the code
  exists. Express separates permission-to-modify (ungated: reversible local work) from
  permission-to-publish (the gate), and the lane stays bounded to non-sensitive reversible change
  because that trade only holds when discarding the work costs little.
- **`agents/orchestrator.md` is ordered by execution** — 49 sections in four bands, with Intake and
  Specify no longer at position 43 of 49, after every phase they precede. Zero content change,
  verified as a line-multiset comparison.
- The nonce binds the gate *presentation*, not the operator's wording. Requiring the reply to carry
  it literally made an operator who followed the STOP block's own instructions produce a reply the
  contract called invalid, with the re-presentation offering the same instructions — a loop with no
  exit.
- Retired `type_reclassify` / `tier_promote` / `tier_promote_rationale` in favour of one shape:
  `failure_kind: reclassification-needed` plus `recommended_type` / `recommended_tier`, `rationale`
  and `evidence`. A wrong type and a wrong tier are the same situation.
- `implementer`: one comment contract with **default: add no comment**, replacing three scattered
  rules plus a 200-line reference loaded on every dispatch; follow-up hunting forbidden; no test or
  documentation authoring (declared instead); no plan writes; and an output template that stops
  re-narrating the plan.
...
- **A rule governing what may be registered as a test** (`README.md § "What gets a test"`). A test
  asserts a property of executable code or a machine-readable artifact, evaluated by running it. The
  diagnostic is: *if this failed, would the cheapest fix be to reword a sentence?* If yes, it is a
  text assertion and it is not registered. Enumerates the forbidden shapes — phrase presence, byte
...
- **A central failure taxonomy** (`agents/ref-pipeline.md § Failures`): nine observable causes, each
  ...
  missing decision and more attempts cannot produce one.
- `agents/_shared/orchestrator-state.md` — the `00-state.md` field schema, the events schema, the
  decision ledger, the pipeline summary, stage-end notifications, the transition protocol, artifact
  verification and the terminal close, read at three named points and never preloaded at boot.
- `reviews/01-closure-rubric.md` — the architect's closure tables get their own single-owner file,
  removing it as a foreign writer of the panel's shared review artifact.

### Removed

- **The prose-assertion test corpus — 46,554 lines across 16 files.** Seven suites asserting the
  presence, wording, byte-shape or count of prose in `agents/` and `skills/`, their fixtures and
  baseline, the behavioral tests whose pass condition was a model self-reporting compliance, the
  behavioral tests pinned to retired architectures, and two files that existed only to feed a
  deleted check. **Stated as a cost, not a cleanup:** the corpus inverted authority — a failing
  literal search made adding a sentence the cheapest fix, so the specification stopped governing the
  prose and the prose started serving the check, and a contradiction could sit in a file with every
  check green. Agent prose now has no mechanical guard; its correctness rests on review.
  `docs/testing.md` drops from 1,027 lines to 96.
...
- **The read-driven / mechanical distinction as a file-editing routing device.** Replaced by an unconditional read obligation per site plus an explicit R1–R5 treatment-rule annotation at every edit (`01-plan.md § "The treatment vocabulary"`). The three checks originally planned to back it — a surviving-mention allowlist, a dangling-pointer guard, and `qa`'s non-loss inventory re-derivation — were retired with the prose-assertion suite (see below); the treatment annotation and review are what remain.
...
### Changed
- Phase 3 verify is now `tester` + `qa` only; the security lenses left the per-task verify block and its iteration loop. The Case → routing table drops Case D (security-only). No security-lens re-run exists in patch iterations — a mid-iteration security concern is carried forward as audit context, never a re-dispatch.
...
- CLAUDE.md trimmed several §5/§6/§8 bullets to their essential statement, moving extended detail to their already-referenced `docs/` files (`docs/dev-mode.md`, `docs/cost-and-caching.md`, `docs/reasoning-checkpoint.md`), bringing the file back under the 35 KB soft cap (§7b).

### Removed
- `adversary_floor_applies` predicate and the `changes_security_control` dispatch conjunct (the field remains as an informational classification signal); per-round `reviews/04-adversary-r{N}.md` reports, the per-round report-integrity scan, and the `adversary_rounds`/`adversary_round_sizes` state fields; the security-verdict staleness re-gate (the audit's end-position over the final diff makes staleness structural). Suites 165-168 retired, superseded by Suite 171 (rationale: the pinned contract was retired by operator decision — this also resolves issue #500 structurally by removing the multi-site dispatch-decision surface).
...
### Added
- `gate-guard`, a new deterministic PreToolUse `Bash` hook (structural sibling of `prepublish-guard`) that closes the outward-action ORDER gap reported in issues #491/#495: it resolves the governing pipeline lane by mtime-selecting the active `00-state.md` and correlating the current branch/worktree against that lane's `working_branch` field — valid in both the worktree and the branch-in-place topology — then denies a `git push`/`gh pr create` from that lane unless `gate3_release: ship` is registered. It also denies, unconditionally on `gate3_release`, an in-lane force-push in either form (a `-f`/`--force`/`--force-with-lease` flag or a `+`-prefixed refspec). Its decision set is `{none, deny}` only — it never emits `ask`, and it defers (`none`) when no governing lane resolves (a manual push, an inline session, or an unrelated repository).
- `agents/_shared/gate-contract.md § "Outward-action release floor"` — the canonical clause for the new invariant, plus a `gate_nonce` field on every dual-record gate: a fresh, single-use token embedded when a gate is prepared, carried through the leader relay, and verified/consumed on release, closing the STAGE-GATE-1 spawn-payload pre-declaration vector reported in issue #491.

### Changed
- `agents/orchestrator.md` splits Delivery into `mode: prepare` (Phase 4a — branch, commits, version, changelog, PR-body draft, entirely local) and `mode: publish` (Phase 4b — push + `gh pr create`, dispatched only after STAGE-GATE-3 records `gate3_release: ship`). This reorder is what lets STAGE-GATE-3's approval *authorize* the push rather than *ratify* one that already happened, and is the reordering `gate-guard` depends on to avoid a deadlock.
- `agents/delivery.md` adopts the matching `mode: prepare`/`mode: publish` split and documents its force-push promise as backed by the new deterministic floor.
- `agents/leader.md` documents the no-pre-declaration invariant (a spawn payload never carries a gate decision) and the `gate_nonce` carry mechanics through the gate mediation/presentation protocol.
- `docs/dev-mode.md`, `docs/reasoning-checkpoint.md`, `docs/pipeline-lanes.md`, `docs/subagent-orchestration.md` document `gate-guard`'s contract, its deny-vs-ask distinction from `dev-guard`, its lane-uniformity coverage across the three execution lanes, and its coverage of the takeover/inline dispatch path.
...
- The `/dev/tty` guard in all four redirect sites across the three bootstrap scripts (`bin/install.sh:72`, `bin/install-opencode.sh:144` and `:151`, `bin/update-opencode.sh:186`) now tests openability (`(exec < /dev/tty) 2>/dev/null`) instead of mere existence (`[ -e /dev/tty ]`) — a shell without a controlling terminal (agentic Bash, CI, cron) where `/dev/tty` exists but cannot be opened no longer fails with "No such device or address" after the download and checksum verification already completed; it falls through to the existing no-redirect branch instead. The originally-reported site was `update-opencode.sh` only; the fix extends to the three sibling sites sharing the same defect (#473).
- Corrected a stale premise carried by the original issue #495 report: `gh pr create` was already covered by `dev-guard` (`ask` by default, `allow` under the `autogate.pr_create` opt-in) before this change — that coverage is preserved unchanged, and the fix documented here is the outward-action ORDER floor, not new `gh pr create` coverage.
- Fixed a router false positive shared by `dev-guard` and `gate-guard`: a covered-action literal (e.g. `git push`, `gh pr merge`) appearing only inside an inert quoted string (a read-only `grep "git push" file`) satisfied the routers' boundary-character class and wrongly triggered `ask`/`deny`. A new shared pre-pass (`hooks/ts/bodies/command-lexer.ts`) blanks balanced, unwrapped quoted spans before the routers run, leaving true-positive invocations (bare, piped, chained, or wrapped through a shell) unaffected.
- Replaced `gate-guard`'s force-push detection — a character-denylist defeated three times by independent adversarial review (whole-token quoting, mid-token quote-splicing, then brace expansion/backtick substitution) — with a shared closed positive grammar (`command-lexer.ts::matchBenignPushGrammar`): only the exact benign push shape (`git push [-u|--set-upstream|-v|--verbose|--progress] origin <plain-branch>`, every character in `[A-Za-z0-9 _./-]`) is permitted in-lane, and everything else is denied by construction rather than by enumeration. `dev-guard`'s Step 0 push char-gate adopts the same shared predicate, closing a latent brace-refspec double-force bug (`git push origin {+,+}feature:otherbranch`) that previously resolved to `allow`.

## [2.131.0] - 2026-07-17

### Changed
- `prepublish-guard.ts` (Check 1) collapses to a single universal invariant on any branch touching a distributed asset (`agents/`, `skills/`, `hooks/`): all three version sites (`.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json` `plugins[0].version`, `CLAUDE.md §3`) must be bumped versus `origin/main` and mutually match when `CLAUDE.md §3` is parseable; the guard retains its documented fail-open behavior otherwise, subject to the existing mechanical SemVer floor (`bump-override` token still required for an over-bump). The prior release-branch discriminator, the `release-cut` marker/trailer recognition, and the feature-path fragment/marker escape hatch are retired — deleted, not commented out (`RELEASE_BRANCH_RE`, `resolveBranch`, `RELEASE_CUT_*`, `FRAGMENT_RE`/`MARKER_RE`/`hasFragmentOrMarker`, `runFeaturePath`, `gitCurrentBranch`). Under-bump remains advisory-only (`WARN`, never `deny`) on every branch, unchanged from the prior release-path behavior — documented and tested as an accepted, named residual.
- `agents/delivery.md` adopts the per-PR version-bump model for team-harness itself: the shipped consumer-facing "Per-PR bump (shipped default)" row, its gates, and its output contract are unchanged for every other repo. `version.d/` markers and the `release-mode`/`inline-release` opt-in modes are removed (no invoker remains once `/th:release` is deleted); `skip-version` and `changelog.d/`+Step 9e remain intact as consumer capabilities. Step 11.4c's tag-verify gate now fires on any per-PR bump in a repo that carries `.github/workflows/tag-sync.yml`, rather than only under `release-mode`/`inline-release`. Step 9.4a's cross-site version MATCH check now runs unconditionally against the site set discovered by Step 9.0, instead of being gated on a given PR's plan declaring a `Multi-site invariants` table — closing a defense-in-depth backstop that previously only fired when a plan happened to restate it.
- `CLAUDE.md §6.3` "Internal distribution rule" rewritten to describe team-harness's own per-PR three-site bump + direct `## [X.Y.Z]` CHANGELOG section as its rule, replacing the deferred-release/`/th:release` framing; documents the rebase-and-rebump trade-off for two PRs racing on the same version line; keeps `changelog.d/` referenced as the batch/fallback path for repos that group releases, not team-harness's own default.
- `docs/worktree-discipline.md` gains **Rule 7 — Boot-time preflight sweep**, the canonical, single source of truth for a 4-condition worktree-removal safety predicate (excludes the active session's worktree and the main tree; requires pipeline provenance; requires the branch merged to `origin/main` AND no commits ahead of the merge point; requires clean-beyond-a-mode-only-diff allow-list), an action/report table (removed / left-dirty / left-unmerged-or-commits-ahead / candidate-unknown-provenance, never a silent skip), a per-worktree atomicity discipline that minimizes (does not eliminate) the residual TOCTOU window, and a `mkdir`-based directory lock (central under `--git-common-dir`, holder-ID diagnostics, 15-minute stale expiry, fail-safe defaults) that serializes cooperating sweepers. `agents/leader.md` Phase 0a gains step "1a", a boot-time preflight sweep that runs Rule 7's predicate by reference before any task fan-out. `agents/delivery.md` Step 11.4b's same-session teardown adopts Rule 7's full 4-condition predicate and lock protocol by pointer. Rule 3's prior claim that teardown "fires in the delivery agent, post-merge" is corrected: delivery's teardown is same-session best-effort only (delivery runs pre-merge); the durable reaper is the leader's boot-time sweep.
- `tests/test_agent_structure.py` retargeted to the models above: Suite 155's changelog check reads the durable versioned `## [X.Y.Z]` CHANGELOG section directly instead of attempting a `changelog.d/` fragment first; Suite 125's retired feature-path deny-reason assertion is retargeted to the new universal-path deny reason; Suite 132 no longer asserts against the deleted `skills/release/SKILL.md` and its retired local-apply checks, and its tag-verify gate assertion is retargeted to the new "any per-PR bump + `tag-sync.yml` present" wording; `_s59_claude_current_version` is tightened to the guard's exact `CLAUDE_VERSION_RE` literal shape, closing a blind spot where a `CLAUDE.md §3` reformat could silently break the guard's third-site parsing while a looser CI check kept passing.

### Removed
- `skills/release/SKILL.md` (`/th:release`) and `version.d/.release-cut` — the deferred-release cut mechanism, superseded by the per-PR bump model above. The one non-duplicated capability (`/th:release`'s local-apply step) already lives in `/th:update`.

### Security
- Closed a defense-in-depth gap where `agents/delivery.md` Step 9.4a's cross-site version MATCH check only fired when a given PR's plan restated the multi-site invariants table, rather than unconditionally on every per-PR bump — a `CLAUDE.md §3` reformat could previously slip through a PR whose plan omitted that table. Tightened `tests/test_agent_structure.py::_s59_claude_current_version` to the guard's exact version-line regex so a reformat that would silently break the guard's parser also fails CI.
- Hardened the boot-time worktree preflight sweep (`docs/worktree-discipline.md` Rule 7) against a check-then-act race in its worktree-removal safety predicate: added a per-worktree atomicity discipline (adjacent re-check immediately before removal, one worktree at a time) and a `mkdir`-based directory lock that serializes the boot-time sweep against `agents/delivery.md`'s same-session teardown. Also fixed a force-repair path (the pre-existing Windows file-lock repair sequence, reused by both automated call sites) that had been unconditionally escalating to `git worktree remove --force` on any initial-remove failure without re-checking dirtiness first — both automated sites now re-verify the tree is still clean immediately before any forced removal, and the check-then-force sequence is collapsed into a single shell invocation to minimize the residual timing window to OS-level command-sequencing latency. Residual: this narrowed window covers cooperating processes only, not an out-of-band concurrent human editor of the same worktree — accepted as the practical floor for this local dev-tooling feature.
...
- The distributed read-only allowlist is verified disjoint from the dev-guard's own outward-action catalogue by a new catalogue-driven test (`tests/test_permission_disjointness.py`) that derives the outward-action patterns directly from `hooks/ts/bodies/dev-guard.ts` and asserts no allow-prefix is a prefix of any outward-action sample, with a canary asserting the test does not pass vacuously. No form of `gh api` (including `gh api graphql`) is ever included in the allowlist, since no `gh api` prefix is disjoint from its own mutating forms.
- Accepted residual: the dev-guard's default-branch resolution trusts `origin/HEAD` for repos with a non-standard default branch; the operator accepted this residual, with `git remote set-head origin -a` documented as the recovery path when `origin/HEAD` drifts from the actual default.
...
- `agents/ref-intake-flows.md`: new on-demand reference file (mirrors `ref-special-flows.md`) holding the five conditional Phase 0a intake sub-flows — milestone continuity, initiative create-or-join, initiative detection + confirm, language/english-learning-set intent handling, and the ClickUp conversational-intents flow — each triggered from a short pointer left in `agents/orchestrator.md`.
...
- **Orchestrator boot reduction (A7).** The dedicated boot dispatch-probe subagent is removed — Task availability is verified by the run's first genuine dispatch instead, with identical Dispatch-blocked-exit semantics. The Phase 0a CLAUDE.md re-read is now conditional on the absence of the top-level session's `claudeMd` injection marker (mandatory only on subagent/takeover/compaction paths). Five conditional Phase 0a intake sub-flows move verbatim to the new `agents/ref-intake-flows.md`, replaced in the spine by 1-2 line triggers; tier tables, the security-sensitive path list, and the Discover/checkpoint (B1/B2/B3) machinery are unchanged.
...
- `agents/researcher.md`: add `Write` to `tools:` allowlist — fan-out lanes write `research-findings-{angle}.md` but the PR #321 shipped frontmatter lacked `Write`, causing a silent tool-denied failure at runtime. `tests/test_agent_structure.py` Suite 92 gains a mandatory regression assertion (`suite92(ac3-researcher-tools)`) that fails on the pre-fix tree and passes on this PR.
...
- **Developer mode — top-level orchestrator with a precondition model.** Pipelines run ONLY when developer mode is active; **without it the top-level agent handles requests DIRECTLY** (no orchestrator, no pipeline, no workspace). This inverts the prior "default to team-harness flows" (#242) into **"default direct / dev mode is the opt-in"**. When active, the top-level agent has Task and dispatches leaf agents directly (no `dispatch_handoff`). The `orchestrator-dispatch-rule` managed block (+ byte-identical mirrors) and CLAUDE.md §14 carry the precondition; `docs/subagent-orchestration.md` takeover is reframed as the FALLBACK for explicit out-of-dev-mode invocation. The observable marker `~/.claude/.dev-mode-active` (`dev_mode: true`) is the single source of truth. Security floors (HI-2, path-pattern auto-escalation, bug-fix forcing rule) are input-independent of dev mode and non-waivable. Contract: `docs/dev-mode.md`.
- **`/dev-mode` skill — in-session toggle, no reload.** A user-level skill (`~/.claude/skills/dev-mode/`, installed by `/th:setup`, synced by `/th:update`) that STARTS developer mode in the current session immediately: it writes the marker, shows a one-line mode banner, and adopts the orchestrator disposition — **no `/clear` required**. `/dev-mode off` removes the marker (operator-confirmed). Contract loading is deferred until a task enters the pipeline, so activation is fast; the marker is written unconditionally on every activation so re-enabling after `/dev-mode off` reliably persists.
- **`SessionStart` hook (`hooks/dev-mode-session-start.sh`) — instant, app-rendered banner + auto-resume.** When the marker is present, the hook (matcher `startup|resume|clear`; wired in `.claude-plugin/hooks.json` for plugin installs and `hooks/config.json` for the Go installer) surfaces the `DEVELOPER MODE ACTIVE` banner via **`systemMessage`** — rendered by Claude Code itself, so it is **instant** (the model-rendered ASCII banner cost ~13-20s and is removed from the fast path) and visible at session start — and injects the orchestrator disposition SILENTLY via `additionalContext` (no banner render, no narration, no marker re-inspection). Every new session auto-resumes dev mode with no `/dev-mode` re-invocation. The full ASCII wordmark remains in the `developer-mode` output style (`output-styles/developer-mode.md`, `keep-coding-instructions: false`) as an opt-in persistent path via `/config`.
- **Deterministic outward-action gate (`hooks/dev-guard.sh`).** A `PreToolUse`/matcher-`Bash` hook, fail-CLOSED, that — when dev mode is active — gates outward/irreversible actions BY DESTINATION (`git push`, `gh pr merge`/`review`/`comment`, `gh api`/`curl`/`wget` with a mutating method against `api.github.com`/`pulls/...`, across `gh`/`git -C`/etc.) and authorizes via human `permissionDecision: "ask"` (the agent cannot self-approve — no forgeable marker). Marker **removal/disabling** is `ask` (so `/dev-mode off` deactivates with operator confirmation, while the agent cannot silently disable the gate), but marker **activation writes** (`dev_mode: true`) are allowed without a prompt so re-activation persists reliably; outward actions are checked before marker manipulation so a combined command surfaces the outward intent. The safety floors are prompt-independent hooks, so they survive `keep-coding-instructions: false`. In dev mode the reasoning-checkpoint is promoted from the Layer-2 self-check to the Layer-1 hook. Anchored by `tests/test_dev_guard.sh` (22 cases) + Suite 59.
...
- **Reasoning-partner posture** — reframes the orchestrator's checkpoint disposition from restraint to engagement: disagreement is authorized and expected (triggered by unclear idea or codified-standard violation, not constant); disagreement is grounded in CLAUDE.md §6/§5 (legible, defensible, never the model's taste); win-condition is "developer reached clarity + idea meets bar + understands why" (not "produced the artifact"); pedagogy clause exposes the WHY without forcing a Socratic march, bounded by "still work, never a seminar, never blocks delivery"; concise-engagement counterweight: surface only the salient friction + decision-relevant why, keep the rest internal (aligns with CLAUDE.md §7.1 output-discipline). Posture text encoded in `agents/orchestrator.md` Step 6d and `docs/reasoning-checkpoint.md § Postura`; Suite 56 presence-checks anchor-scoped in `tests/test_agent_structure.py`.
...
[Unreleased]: https://github.com/valianx/team-harness/compare/v3.20.5...HEAD
[3.20.5]: https://github.com/valianx/team-harness/compare/v3.20.4...v3.20.5
